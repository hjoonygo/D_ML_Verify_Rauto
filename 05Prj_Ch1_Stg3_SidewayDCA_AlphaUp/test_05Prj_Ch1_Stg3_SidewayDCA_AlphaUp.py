# -*- coding: utf-8 -*-
# [파일명] test_05Prj_Ch1_Stg3_SidewayDCA_AlphaUp.py
# 코드길이: 약 470줄 | 내부버전: SidewayDCA_AlphaUp_05_Ch1_Stg3 (펀딩전체+손절깊이) | 로직 축약/생략 없이 전체 출력
# ─────────────────────────────────────────────────────────────────────────────
# [이 코드가 하는 일 — 고딩 설명]
#   stg9 봇(얕은 POC 되돌림 + S_TREND 게이트 + 피보청산)을 기반으로, 측정 정직화(stg1) + 왕복비용(stg2)에 더해
#   stg3에서 두 가지를 손본다(펀딩 정확화 + MDD의 진짜 범인 sl_deep 손절깊이 조절).
#   유지(stg1·2): ①1분봉 경로 청산  ②실펀딩(부호반영)  ④왕복비용(COST_SIDE*2)
#   신규(stg3):
#     ⑤ 펀딩 전체본 적용 : 바이낸스 펀딩의 밀리초+TZ 혼재 포맷을 format='ISO8601'로 강제 파싱(stg2는 일부만 읽힘).
#                          전체본(BTCUSDT_funding_history_8h.csv)을 샘플보다 우선 탐색. 로드 커버율 진단·경고.
#     ⑥ sl_deep 손절깊이 : 'POC에서 sl_mult×ATR 벗어나면 sl_deep' — 손절깊이를 진입(dist_max=1.5)과 분리해 조절.
#                          stg2에서 '거래당 고정% 손실캡'은 되돌릴 거래를 일찍 죽여 MDD가 오히려 커지는 역효과 확인됨.
#                          그래서 캡 폐기 → 레벨식 손절깊이 sl_mult를 5단계(1.2/1.5/1.8/2.2/2.6) 스윕해 최적 탐색.
#   목표: HONEST 무캡에서 MDD -17.9%(폭주선 -15% 초과)였던 것을, sl_mult 조절로 -15% 안에 넣을 수 있는지 검증.
#
# [★사용명칭 정의 — sl_mult 의 뜻]  ※추정 방지 위해 명시
#   sl_mult = 'POC 기준 sl_mult×ATR 만큼 가격이 벗어나면 깊은손절(sl_deep)' 하는 손절깊이 배수.
#   작을수록(1.2) 빨리 자름 → 한 방 손실↓ 이지만 되돌릴 걸 일찍 죽여 휩쏘↑.
#   클수록(2.6)  늦게 자름 → 되돌림 기회↑ 이지만 한 방 손실↑.
...
#
# [PATH] 실행: D:\ML\verify\05Prj_Ch1_Stg3_SidewayDCA_AlphaUp\ . 데이터/펀딩: 상위 D:\ML\verify\ .
...
# ── stg3 설정: 펀딩 전체본 + sl_deep 손절깊이 스윕(캡 방식 폐기) ──
...
#   sl_mult가 작을수록(1.2) 빨리 자름→손실↓·휩쏘↑, 클수록(2.6) 늦게 자름→손실↑·되돌림 기회↑.
#   stg2에서 캡(진입가 대비 고정%)은 되돌릴 거래를 일찍 죽여 역효과 확인됨 → 레벨식 손절깊이로 대체.
SL_MULT_GRID = [1.2, 1.5, 1.8, 2.2, 2.6]
DEFAULT_SLMULT = 1.5               # 무캡(stg2)과 동일선상 비교용 기준값(=진입과 같은 깊이)
...
# ── 정직 엔진(1분봉 경로 청산 + 실펀딩 + sl_deep 손절깊이 sl_mult) ──
def run_bot_honest(df, sig, par, mO, mH, mL, mT, ss, se, ft, fr, sl_mult):
    ...
    while j < n:
        ...
        if pos != 0:
            ...
            #     stg3 핵심: 손절깊이를 진입(dist_max)과 분리해 sl_mult로 조절(캡 방식 폐기).
...
def main():
    print("[SidewayDCA_AlphaUp_05_Ch1_Stg3] 펀딩전체본 + sl_deep 손절깊이 스윕(캡 폐기)")
    ...
    # (B) 정직 엔진 기준선(sl_mult=기본1.5, stg2 무캡과 동일선상) + 인트라바 모호도 측정
    tr_h0, ambig_n, held_n = run_bot_honest(df8, sig, BEST_PAR, mO, mH, mL, mT, ss, se, ft, fr, DEFAULT_SLMULT)
    ...
    rows.append({'cell': f'HONEST_base(slmult={DEFAULT_SLMULT}·1m경로·실펀딩)', 'cap_pct': DEFAULT_SLMULT,
    ...
    # (C) sl_deep 손절깊이 sl_mult 스윕 (캡 방식 폐기, 레벨식 손절깊이로 대체)
    for sm in SL_MULT_GRID:
        tr_c, _, _ = run_bot_honest(df8, sig, BEST_PAR, mO, mH, mL, mT, ss, se, ft, fr, sm)
        ...
        rows.append({'cell': f'HONEST_slmult_{sm}', 'cap_pct': sm, 'full_trades': ac['trades'],
    ...
    verdict = (f"VERDICT stg3 펀딩+손절깊이 | OLD PF={a_old['PF']} cumR={a_old['cumR_pct']}% MDD={a_old['MDD_pct']}% "
               f"-> HONEST(base) PF={a_h0['PF']} cumR={a_h0['cumR_pct']}% MDD={a_h0['MDD_pct']}% WFE={wfe_h0}% "
               f"| 인트라바모호도={ambig_pct}% | 펀딩={fund_note} | 추천={cap_msg}")
