# -*- coding: utf-8 -*-
# [FILE] check.py  (SpTrd_Fib_V0_stg7 - contamination 8-check + verdict + INDEX)
# CODE LENGTH: approx 190 lines | INTERNAL VER: check_sfstg7 | full output, no omission
#
...
SIZING = "sfstg7_sizing.csv"; TRADES = "sfstg7_trades.csv"
ADXF = "sfstg7_adx.csv"
EXPECTED = [SIZING, ADXF, TRADES]
...
def run_checks(start_ts):
    ...
    found = [os.path.basename(f) for f in glob.glob(os.path.join(HERE, "sfstg7_*.csv"))]
    ...
    if os.path.exists(paths[ADXF]):
        try:
            s = pd.read_csv(paths[ADXF])
...
def write_analysis(checks, analysis):
    ...
    verdict_lines = []
    # (1) 사이징 결과
    sp = os.path.join(HERE, SIZING)
    if os.path.exists(sp):
        try:
            sz = pd.read_csv(sp)
            # 진짜파산 NO인 것 중 CAGR 최대
            ok_sz = sz[sz['진짜파산'].astype(str) == 'NO'].copy()
            ok_sz['c'] = pd.to_numeric(ok_sz['CAGR_pct'], errors='coerce')
            if len(ok_sz):
                best = ok_sz.sort_values('c', ascending=False).iloc[0]
                verdict_lines.append(f"최적사이징(파산X, CAGR최대): {best['사이징']} "
                                     f"CAGR{best['CAGR_pct']}% 월{best['월환산_pct']}% MDD{best['MDD_pct']}%")
            # S0 고정(착시 기준)과 파산 개수
            nb = int((sz['진짜파산'].astype(str) == 'YES').sum())
            verdict_lines.append(f"사이징 8칸 중 진짜파산 {nb}칸 (나머지는 파산착시였음)")
        except Exception as e:
            verdict_lines.append(f"(사이징 추출 실패: {e})")
    # (2) ADX 고원
    ap = os.path.join(HERE, ADXF)
    if os.path.exists(ap):
        try:
            ad = pd.read_csv(ap)
            plat = (pd.to_numeric(ad['PF_test'], errors='coerce') > 1.0).all()
            parts = " / ".join(f"ADX{int(r['ADX임계'])}:test PF{r['PF_test']}" for _, r in ad.iterrows())
            verdict_lines.append(f"ADX 고원확인: {parts} -> {'고원(견고)' if plat else '봉우리(우연의심)'}")
        except Exception as e:
            verdict_lines.append(f"(ADX 추출 실패: {e})")
    ...
    with open(fpath, 'w', encoding='utf-8') as f:
        ...
        f.write("=" * 60 + "\n[알파 판정 (SpTrd+Fib stg7 Sizing+ADX)]\n")
