...
#
# [목적] 통합판_v2 문서 명시 수정(하드플로어 항상 ON)을 적용한 엔진(Exec_GridD_v2)으로
...
#
# [수정 출처] 통합판_v2 "1-5": 손절 구멍(첫 OB 미도달 시 손절 부재) -> 진입 즉시 하드플로어 항상 ON.
#
...
def main():
    ...
    print(f"[수정] 진입 즉시 하드플로어 항상 ON. 그리드 손절 {HARD_FLOORS}%(ROE) × 비율 {[r[1] for r in RATIOS]}\n")
