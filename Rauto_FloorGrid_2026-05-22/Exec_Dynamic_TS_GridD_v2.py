...
class Exec_Dynamic_TS_GridD_v2:
    def __init__(self):
        pass
    ...
    def check_exit(self, current_price: float, bot_state: dict, params: dict) -> dict:
        ...
        # ============================================================
        # [패치 v10c — 구멍일 때만 하드플로어] 통합판_v2 문서 구멍 정의 = "첫 OB 미도달 시 손절 없던 문제".
        #   따라서 하드플로어는 '보호스탑(fib_stop)이 아직 없을 때(=구멍)'에만 작동한다.
        #   OB 타겟을 한 번이라도 찍어 fib_stop이 잡히면 -> OB 엣지 스탑이 지배, 하드플로어는 빠짐.
        #   => 정상 거래(2.864 메커니즘)는 안 건드리고, 구멍 구간의 역방향 폭주만 차단.
        # ============================================================
        if bot_state['fib_stop'] is None:                      # 보호스탑 아직 없음 = 구멍일 때만
            hard_floor_pct = params.get('hard_floor_roe', params.get('fib_sl_roe', 3.0)) / 100.0
            if pos == "LONG":
                hf = entry * (1 - hard_floor_pct / leverage)
                if current_price <= hf:
                    return {"action": "CLOSE_LONG", "reason": f"구멍 하드플로어 손절 (-{hard_floor_pct*100:.2f}% ROE)"}
            else:  # SHORT
                hf = entry * (1 + hard_floor_pct / leverage)
                if current_price >= hf:
                    return {"action": "CLOSE_SHORT", "reason": f"구멍 하드플로어 손절 (-{hard_floor_pct*100:.2f}% ROE)"}
