...
class Backtest_Engine_Downtrend36_v1:
    ...
    def _sim_short(self, e_idx):
        ...
        hole_mae = 0.0   # 구멍 구간(fib_stop=None) 최대역행 ROE%. SHORT: 가격 상승 = 역행(+).
        for i in range(e_idx + 1, end_idx):
            for price in self._ticks(i):
                if bs['fib_stop'] is None:   # 보호스탑 잡히기 전(구멍) 동안만 역행 추적
                    adv = (price - entry_price) / entry_price * self.leverage * 100.0
                    if adv > hole_mae:
                        hole_mae = adv
                sig = self.exec.check_exit(price, bs, self.params)
                act = sig.get('action') if sig else None
                if act == "REDUCE_SHORT" and not reduced:
                    self._rec(bs, e_idx, i, price, position_size, sig['reason'], 'REDUCE', hole_mae=hole_mae)
                    position_size *= (1.0 - self.split_ratio)
                    reduced = True; bs['remaining_pct'] = 1.0 - self.split_ratio
                elif act == "CLOSE_SHORT":
                    self._rec(bs, e_idx, i, price, position_size, sig['reason'], 'CLOSE', reduced, hole_mae=hole_mae)
                    return i
        self._rec(bs, e_idx, end_idx - 1, self.c[end_idx - 1], position_size, 'max_hold_force', 'CLOSE', reduced, hole_mae=hole_mae)
        return end_idx - 1

    def _rec(self, bs, e_idx, x_idx, price, position_size, reason, kind, reduced=False, hole_mae=0.0):
        ...
        self.trades.append({
            ...
            "순수익금($)": round(net, 2), "구분": kind, "구멍MAE_ROE": round(hole_mae, 2)
        })
