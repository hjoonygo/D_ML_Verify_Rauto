# -*- coding: utf-8 -*-
# [파일명] check_06Prj_Ch4_SidewayDCA4RAUTO_Stg6.py
# 코드길이: 약 200줄 | 내부버전: ChampBot_SidewayDCA4RAUTO_06_Ch4_Stg6 | 로직 전체 출력
# [역할] test.py 후: (1)오염검사 8항목 (2)분석txt를 상위 00WorkHstr 저장 (3)INDEX 한줄. 전량 파일로만(복붙 금지).
# [입력] opt_summary.csv / opt_trades.csv / opt_yearly.csv / .opt_metric (실행폴더)
...
VER = "ChampBot_SidewayDCA4RAUTO_06_Ch4_Stg6"
TESTPY = "test_06Prj_Ch4_SidewayDCA4RAUTO_Stg6.py"
CHECKPY = "check_06Prj_Ch4_SidewayDCA4RAUTO_Stg6.py"
REQUIRED = [TESTPY, CHECKPY, "run.bat", "opt_summary.csv", "opt_trades.csv", "opt_yearly.csv"]
...
def parse_verdict():
    sp = os.path.join(HERE, "opt_summary.csv")
...
def read_metric():
    p = os.path.join(HERE, ".opt_metric")
    d = {}
    if os.path.exists(p):
...
def check_all():
    ...
    csvs = ["opt_summary.csv", "opt_trades.csv", "opt_yearly.csv"]
    ...
    dup = 0; tp = os.path.join(HERE, "opt_trades.csv")
    if present.get("opt_trades.csv"):
        try:
            t = pd.read_csv(tp)
    ...
    if has_oi:
        # Stg6: 연도별 일관성 매트릭스(opt_yearly)에 연도×ER 행이 채워졌는지
        mlp = os.path.join(HERE, "opt_yearly.csv")
        ok6 = False; memo6 = "opt_yearly 없음"
        if os.path.exists(mlp):
            try:
                mj = pd.read_csv(mlp)
                ok6 = ("better" in mj.columns) and (len(mj) >= 4)
                bw = int(mj["better"].sum()) if "better" in mj.columns else 0
                memo6 = f"연도×ER {len(mj)}행, 원본대비우위 {bw}건"
            except Exception as e:
                memo6 = f"읽기실패:{e}"
    else:
        ok6 = True; memo6 = "OI없음 → 검증불가(상위 D:\\ML\\verify\\Merged_Data.csv 확인)"
    res.append(("6.연도일관성기록", ok6, memo6))

    scen_ok = False; sp = os.path.join(HERE, "opt_summary.csv")
    if present.get("opt_summary.csv"):
        try:
