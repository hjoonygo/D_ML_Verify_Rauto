...
| 🔴 | **★미시레짐 세분 → REVoi 레짐적응**(캡틴 통찰 2026-06-28·선행연구 검증완료) | 7일추세 3분류(상/하/횡)가 거침=이름표 괴리(COMBO 하락 42.6% in-sample vs 최근 5/12~6/28 PF0.8·+19% OOS). 미시특성(휩소율·짧은 급변·변동성 죽은 횡보·점프강도·실현변동성·ADX) 세분 분류 → REVoi(평균회귀형=급락강·휩소약) 레짐적응 사이징/전환. 선행연구=HMM·변동성레짐·크립토점프58%·choppy step-aside 지지 | held-out·CPCV 표준6 전폴드 통과 + 이름표 OOS 정합 + 환각0 | `00_Basic_Setup_Package/260627_02_Rauto2_Key_ComboLoad_NameplateTradeLog.md` §8 |
...
| 🔴 | **추세봇 포트폴리오** | REVoi와 상관 낮은 추세추종 알파를 봇계약(make_trades)으로 제작 → REVoi와 포폴 → MDD−20 인증 재타지. (260627_01 세션 대결론=단일 REVoi론 MDD−20 불가) | CPCV 표준6 전폴드 MDD≤−20·p25>0 + REVoi와 상관 낮음 입증 + 환각0 | `00_Basic_Setup_Package/260627_01_RegimeCertCard_Key_RotationWhipsawFindings.md` |
