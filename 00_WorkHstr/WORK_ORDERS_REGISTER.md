...
| 🟡 진행중(260702_01) | **★실시간 미세레짐 분류 + 휩소 내성**(캡틴 지시 2026-07-01) | ★진척(260702_01_MicroRegimeWhip): 진단(Stg1 손실지도·causal 8피처)=휩소하락 아니라 **랠리 역주행이 진짜 저EV 병목**(gross손실39%). **L2 랠리억제**(상승7일추세≥+3%서 숏 노출×0.5·비대칭)=held-out OOS 수익2배(+1058→+2155%)·같은노출선 리스크리듀서(MDD-14.8→-12.6) → 봇무관 PlugIn `engines/rauto_regime_sizing.py` + **RevoiRally@ETF** 등록·무손상. ★잔여=**매월양수100%·MDD-20 미달(단일알파 4차확인)→추세봇 상보 선결**. | held-out·CPCV 전폴드 MDD≤-20 + 매월양수↑ (추세봇 상보와 함께) | `00_Basic_Setup_Package/Work_Order_MultiAlpha_RegimeChampion_20260628.md` + `03_IDEA4Bot/260702_01_MicroRegimeWhip/` |
