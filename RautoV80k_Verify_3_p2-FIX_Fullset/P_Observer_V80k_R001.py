...
if not os.path.exists(os.path.join(BASE_DIR, "PautoV80_TBM_BULL_v2.json")):
    ...
    if os.path.exists(os.path.join(_ws, "PautoV80_TBM_BULL_v2.json")):
        BASE_DIR = _ws
    else:
        ...
        if os.path.exists(os.path.join(_ext, "PautoV80_TBM_BULL_v2.json")):
            BASE_DIR = _ext
        else:
            # 4순위: 기존 fallback
...
try:
    from Observer_Logger import log_observation, fill_horizon_labels
    _LOGGER_OK = True
except Exception:
    _LOGGER_OK = False
...
def get_signal(df: pd.DataFrame, current_regime: str, params: dict) -> dict:
    ...
    try:
        ...
        # ★ V80k_Verify_3_S15a: 사후 라벨 산출 (P_Observer Bot_2.csv 누락 버그 fix)
        # 현재 봉이 30분 전 봉의 사후 데이터 — 30분 전 record에 label_class 채우기
        # (R_Observer에만 있던 호출을 P_Observer에도 추가)
        try:
            current_high = float(work_df['high'].iloc[-1])
            current_low = float(work_df['low'].iloc[-1])
            current_close = float(work_df['close'].iloc[-1])
            fill_horizon_labels(bot_id, last_ts, current_high, current_low, current_close,
                               horizon_minutes=30)
        except Exception:
            pass  # 사후 라벨 실패해도 거래 영향 X
