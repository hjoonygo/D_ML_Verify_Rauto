...
# ★콘솔 인코딩 무관하게 출력 — 서비스/리다이렉트(stdout=cp1252/cp949)서 ★·한글 print 크래시 방지(코드 자체로 보장)
...
# ── self-locating: 실행 폴더와 무관하게 필요한 폴더·파일을 스스로 찾는다 (★캡틴 지침 2026-06-20) ──
#   env 있으면 env 우선, 없으면 서버위치·드라이브를 stat으로 탐색. 못 찾으면 관례 기본값. (하드코딩 절대경로 의존 제거)
_DRIVES = []
for _dv in ("C:\\", "D:\\", os.path.splitdrive(HERE)[0] + "\\"):
    if _dv not in _DRIVES:
        _DRIVES.append(_dv)


def _find_repo():
    env = os.environ.get("RAUTO_REPO", "")
    if env and os.path.isdir(env):
        return env
    marker = os.path.join("06Prj_Ch8_Plugin_Stg1_TS_Impatient", "rauto1", "test_Rauto1.py")
    cands, d = [], HERE
    for _ in range(7):                       # 1) 서버 위치에서 위로
        cands.append(d); d = os.path.dirname(d)
    for dv in _DRIVES:                        # 2) 드라이브별 관례 위치
        cands += [dv + "RautoRepo", dv + "RautoRepo\\D_ML_Verify_Rauto", dv + "ML\\Verify", dv]
    for c in cands:
...
def _find_glob():
    env = os.environ.get("RAUTO_STATE_GLOB", "")
    if env:
        return env
    for dv in _DRIVES:
        if glob.glob(dv + "Rauto*\\state.json"):
            return dv + "Rauto*\\state.json"
    return r"C:\Rauto*\state.json"


def _find_flagdir():
    env = os.environ.get("RAUTO_FLAG_DIR") or os.environ.get("RAUTO_DAUTO_DIR")
    if env:
        return env
    for dv in _DRIVES:
        for nm in ("BinanceData", "Rauto1"):
            if os.path.isdir(dv + nm):
                return dv + nm
    return r"C:\BinanceData"


STATE_SRC = os.environ.get("RAUTO_STATE_JSON", os.path.join(HERE, "state_example.json"))
STATE_GLOB = _find_glob()       # 슬롯 state.json 병합 패턴 (자동탐색)
FLAG_DIR = _find_flagdir()      # 제어 플래그 기록 폴더 (자동탐색)
PORT = int(os.environ.get("RAUTO_CTRL_PORT", "8787"))
REPO = _find_repo()             # 봇 러너·git auto-pull 루트 (자동탐색)
...
def aggregate_state():
    ...
    for fp in sorted(glob.glob(STATE_GLOB)):
        try:
            ...
            _m = _re.search(r"Rauto(\d+)", fp)
            _fld = int(_m.group(1)) if _m else 0
            for s in d.get("slots", []):
                s["_folder"] = _fld          # ★슬롯의 실제 폴더번호(제거·식별의 진짜 키, 이름 중복 무관)
            parts.append(d)
        except Exception:
            pass
...
def _load_bot(name, slot_n):
    ...
    try:
        ...
        shutil.copy2(src, dst)
        env = dict(b["env"])
        for kk in ("DUAL_SLOT", "CVD_SLOT"):     # ★슬롯번호를 대상 폴더에 맞춤 → 이름 중복 방지(R{folder}·전략)
            if kk in env:
                env[kk] = "R%d" % slot_n
        _run_env(dst, env)
...
if __name__ == "__main__":
    print(f"[control_server] http://0.0.0.0:{PORT}  (self-locating)")
    print(f"  [paths] HERE={HERE}")
    print(f"  [paths] REPO={REPO or '(not found)'} | glob={STATE_GLOB} | flags={FLAG_DIR}")
    ...
    if REPO and os.environ.get("RAUTO_GIT_PULL", "1") != "0":   # REPO 발견 시 자동 시작(설정 불필요)
        threading.Thread(target=_git_pull_loop, daemon=True).start()
        print(f"[git] auto-pull + bot autoload from {REPO} (AUTOLOAD={AUTOLOAD})")
