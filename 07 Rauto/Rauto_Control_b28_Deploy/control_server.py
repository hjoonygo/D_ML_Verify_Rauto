...
def _load_tokens():
    d = {}
    for pair in os.environ.get("RAUTO_TOKENS", "").strip().split(","):
        pair = pair.strip()
        if ":" in pair:
            t, r = pair.split(":", 1)
            t = t.strip(); r = r.strip().lower()
...
class H(http.server.SimpleHTTPRequestHandler):
    ...
    def _role(self):
        # 역할: admin/view/None. ★인증 OFF(토큰 미설정)면 admin = 기존 동작 호환.
        if not AUTH_ON:
            return "admin"
        return TOKENS.get(self._token())
    ...
    def do_POST(self):
        ...
        if p.path in ("/email", "/cmd") and self._role() != "admin":
...
if __name__ == "__main__":
    ...
    print(f"  [RBAC] auth={'ON' if AUTH_ON else 'OFF(open)'} | tokens={len(TOKENS)} (admin={sum(1 for r in TOKENS.values() if r=='admin')}/view={sum(1 for r in TOKENS.values() if r=='view')}) | audit={AUDIT_LOG}")
