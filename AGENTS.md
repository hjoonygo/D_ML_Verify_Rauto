# AGENTS.md — Rauto 자동매매봇 개발 공통 척추 (D:\ML\verify)
...
## 11. Codex AI(웹) ↔ Codex(PC) 협업 프로토콜 (크로싱=토큰·시간, 최소화가 목표)
- 직접 실시간 통신 채널 없음(분리 세션). 비동기 다리 3개로만 전달:
  ① AGENTS.md = Code 세션 자동로드(반복설명 제거) · ② STATE 1장 = 압축 핸드오버 ·
...
- 역할 고정(한쪽서 끝낼 일 중간에 넘기지 말 것):
  ...
  · Codex AI(웹) = 알파분석·전략·비판검증·문서작성·웹검색.
...
- 실행주체: Codex(앱의 Codex)가 PC에서 D:·G: 드라이브 파일을 직접 읽기·생성·수정·저장하고,
...
- 단일출처: 이 AGENTS.md가 자동로드되는 규칙 단일출처. 새 세션은 이 파일부터 펼친다.
...
- AI 이식성(다른 AI로 교체해도 동일 작업): 새 AI 온보딩 5단계 —
  ① AGENTS.md 정독(규칙) ② STATE_Rauto.txt(현황 1장) ③ 00WorkHstr_INDEX.txt(시간순 Stg)
...
- 규칙: '전환점'이 되는 검증결과는 반드시 5곳에 동시 기재(한 곳이라도 빠지면 다음 세션이 옛값을 정답으로 읽어 사고).
  ① AGENTS.md(규칙·수치 단일출처 — 버전 병기, 삭제금지) ② Hstr_Ver_Up_(봇).docx(ADR·사양 변경)
...
- ★실사례 ADR-001 (비용버전 혼동, 2026-06-15):
  ...
  ⑤결론: 전환점은 시간순기록(INDEX)만으론 부족 — 단일출처(AGENTS.md)+살아있는 사양서에 '버전 병기'해야 다음 세션이 정답을 읽는다.
...
- ★CH(채팅회차)명 폐기 = 의미 없어짐. 의미 단위 = (Proj번호)Prj + CCproject명 + Stg(결과물 작업횟수)_작업명.
  · CCproject = 이 Codex 작업 프로젝트 명칭(캡틴 지정. 예: Rauto_Phone).
