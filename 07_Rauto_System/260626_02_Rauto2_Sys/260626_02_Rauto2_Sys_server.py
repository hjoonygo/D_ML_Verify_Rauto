...
CHAMP_PIN = os.environ.get("RAUTO2_CHAMP_PIN", "RevoiSafe@ETF")      # ★인증봇 고정(캡틴 2026-06-29: RevoiSafe@ETF로 핀 = 안전사이징 봇). ""면 자동선발+안전가산점.
...
BOT_REGISTRY = [
    ...
    {"name": "RevoiSafe@ETF",        "key": "REV_MDD25_36mo", "lev": 15.0, "sz": 20.0,  "tp_frac": 0.7, "early_tp_pct": 0.01, "early_frac": 1.0, "mdd": -14.8, "desc": "★안전사이징 인증봇(캡틴 확정 260629·노출3·lev15·증거금20%)·예상 월복리 16.54%(held-out OOS·현실슬립10bp)·강제청산0·갭관통안전(lev≤17)·조기익절COMBO·한방최대손실20%·MDD-14.8%"},  # ★안전사이징(Stg2~8)·증거금20%=lev3/100%와 수익동일+극단급락 손실cap
    {"name": "RevoiRally@ETF",       "key": "REV_MDD25_36mo", "lev": 15.0, "sz": 20.0,  "tp_frac": 0.7, "early_tp_pct": 0.01, "early_frac": 1.0, "rally_damp": [3.0, 0.5], "mdd": -12.6, "desc": "★휩소내성(L2 랠리억제·260702)=RevoiSafe+상승장(7일추세≥+3%) 역주행(숏) 노출×0.5·예상 월복리 15.93%(held-out OOS·현실슬립10bp·lev15)·같은노출서 MDD -14.8→-12.6% 개선·매월양수↑·강제청산0·조기익절COMBO"},  # ★L2 레짐적응 사이징(rauto_regime_sizing)·리스크리듀서
...
REG_MONTHLY = {
    ...
    "RevoiSafe@ETF": {"up": 13.76, "down": 39.21, "range": 16.65},   # ★안전사이징(260629·노출3·veri_edge.nameplate)·헤드라인 16.54%=OOS·하락 n=1 노이즈·횡보 robust 16.65
    "RevoiRally@ETF": {"up": 10.98, "down": 38.69, "range": 15.31},   # ★L2 랠리억제(260702·노출3+size_mult)·헤드라인 15.93%=OOS·상승 damp로 up↓(휩소내성)·하락 n=1 노이즈
                ...
                for nm in list(self.loaded):
                    ...
                    if b:
                        ...
                        nl.add_bot(nm, REVoiBot(pp), b["sz"], b["lev"], dd_cut=b.get("dd_cut"),
                                   ...
                                   safety_meta=b, rally_damp=b.get("rally_damp"))   # ★비상 안전장치 가산점 + L2 레짐적응 사이징
                with self.lock:
                    self.live = nl
    ...
    def load_bot(self, name):
        ...
        with self.lock:
            ...
            self.live.add_bot(name, REVoiBot(p), b["sz"], b["lev"], dd_cut=b.get("dd_cut"),
                              ...
                              safety_meta=b, rally_damp=b.get("rally_damp"))   # ★비상 안전장치 + L2 레짐적응 사이징(랠리억제)
...
def _champ_report():
    ...
    L.append("  ★비상 안전장치 가산점 %s/8 [%s]" % (
        champ.get("safety", 0), "·".join(it["name"] for it in champ.get("safety_items", [])) or "없음"))
    wk = champ.get("wk", {})
