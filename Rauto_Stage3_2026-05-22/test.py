...
# 코드길이: 약 330줄, 내부버전명: Stage3_no_tp_fibo_only, 로직 축약/생략 없이 전체 출력
#
...
#
# [★결과 전량 파일] S3_diag_*.csv(진단) + S3_trades_*.csv + S3_summary.csv. 복붙 불필요.
...
from Exec_Fibo_v4 import Exec_Fibo_v4
...
TF_LIST = [15, 60]        # OB TF (분). 5분은 표본부족으로 제외
...
def entry_gate_mtf(price, ts, hpc, hpt, hpb, lpc, lpt, lpb):
    ...
    # TP 제거판: 위 저항OB(SL)만 있으면 진입. TP/RR 조건 없음(하락장 지지OB 부재 대응).
    if tp is not None:  # 진단용으로 거리만 기록(게이트엔 미사용)
        res['tp_price']=tp[0]; res['tp_dist']=(price-tp[0])/price
    ...
    if sl_dist > SL_CLAMP: sl_eff=SL_CLAMP
    else: sl_eff=sl_dist
    res['sl_dist']=sl_eff
    if res['tp_dist'] is not None and sl_eff>0:
        res['rr']=res['tp_dist']/sl_eff   # 진단용
    res['pass']=True   # SL만 통과하면 진입
    return res
...
def main():
    print("[Stage3-TP제거] OB+SL/TP=상위TF(5/15/60분), 청산=1분. RR3 고정, 락인0.382")
    ...
    for tf in TF_LIST:
        ...
        pd.DataFrame(trades).to_csv(os.path.join(HERE,f"S3_trades_tf{tf}.csv"),index=False,encoding='utf-8-sig')
    ...
    pd.DataFrame(diag_rows).to_csv(os.path.join(HERE,"S3_diag.csv"),index=False,encoding='utf-8-sig')
    pd.DataFrame(summary).to_csv(os.path.join(HERE,"S3_summary.csv"),index=False,encoding='utf-8-sig')
    print("\n[저장] S3_diag.csv(진단표) + S3_trades_tf*.csv + S3_summary.csv — 전량 파일")
