# -*- coding: utf-8 -*-
# [파일명] test_05Prj_Ch1_Stg6_SidewayDCA_AlphaUp.py
# 코드길이: 약 640줄 | 내부버전: SidewayDCA_AlphaUp_05_Ch1_Stg6 (OI 2차필터+자동병합) | 로직 축약/생략 없이 전체 출력
# ─────────────────────────────────────────────────────────────────────────────
# [이 코드가 하는 일 — 고딩 설명]
...
def main():
    ...
    if has_atrr:
        ...
        # ── stg6 핵심: OI 2차필터 (1차 통과분을 OI급증으로 추가차단) ──
        if has_oi:
    ...
    verdict = (f"VERDICT stg6 OI 2차필터 | 필터:{filt_msg} | 고원 {stab} | ★OI: {oi_msg} "
               f"| 펀딩={fund_note} | atr_ratio={'O' if has_atrr else 'X'} oi_zscore={'O' if has_oi else 'X'}")
    ...
    # trades 저장용 거래원장: stg6 주력 = OI있으면 1차+2차, OI없고 atr만 있으면 1차, 둘다없으면 base
    if has_atrr and has_oi:
    ...
    # trades = stg6 주력 거래원장(필터 가동시 precise)
    ...
    # scenarios = stg6 주력 거래의 8시나리오(train/test)
    ...
    # check.py 가 읽을 메트릭(인트라바 모호도 + stg6 필터 거래라벨)
