...
[코드길이] 약 280줄
[목적] Stage 3 측정 일괄 실행 — 사용자 PC entry point
...
[흐름]
  ...
  4. 모델 backup (재학습 시)
  5. 학습 (--skip-train 가능)
  6. 단위 테스트 (test_v10_stage_3.py)
  7. 측정 (measure_v34_stage_3.py)
  8. 결과 안내
...
def run_unit_tests():
    print_step(5, 7, "Unit tests (v10 Stage 3, ~30 sec)")
    ok, elapsed = _run_subprocess("test_v10_stage_3.py", "Unit tests")
    if not ok:
        return False
...
def run_measurement():
    ...
    ok, elapsed = _run_subprocess("measure_v34_stage_3.py", "Measurement")
    if not ok:
        return False
...
def show_completion():
    ...
    print("\n  Next: zip the entire 'outputs_stage_3' folder and upload.")
...
def main():
    ...
    print_header("Rauto V34 Stage 3 - Dynamic SL (ATR-based)")
    print("  Working dir: " + WORK_DIR)
    print("  Grid: 4 scenarios (SL_MAX 100/120/150/180 bp at OB TF 60m)")
    print("  Rules: Dynamic ATR-based SL replaces fixed 100bp clamp")
