...
#   3 파일명 불일치(오타/버전)      : 고정 패턴 S1b_trades_*.csv / S1b_summary.csv ?
...
TRADES_GLOB = "S1b_trades_*.csv"
SUMMARY = "S1b_summary.csv"
...
def run_checks(start_ts):
    ...
    # [3] 파일명 패턴
    bad = [os.path.basename(f) for f in trade_files if not os.path.basename(f).startswith("S1b_trades_")]
    ...
    # [5][6] 거래 중복 + NaN/0건
    dup_total = 0; empty_files = []; nan_total = 0; per_cfg = []
    ...
    ok5 = (dup_total == 0)
    ok6 = (len(empty_files) == 0) and (nan_total == 0) and ok1
    checks.append(("5.중복없음", ok5, f"중복행 {dup_total}"))
    checks.append(("6.빔/NaN없음", ok6, f"빈파일={empty_files if empty_files else '없음'} NaN={nan_total}"))
    ...
    with open(INDEX, 'a', encoding='utf-8') as f:
        ...
        if all_pass:
            f.write(f"{analysis['time']} | {ZIP_NAME} | 분석:{stamp}.txt | "
                    f"테스트:test.py | 결과:S1b_summary.csv | "
