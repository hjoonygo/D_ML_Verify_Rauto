# -*- coding: utf-8 -*-
# [파일명] test_07Prj_Ch4_RunAWS_Stg14_LivePaperWarmup.py
# 코드길이: 약 270줄 | 내부버전: ch4_stg14_warmup_v4_awsport (v2 완전체 + v4 이식성 패치 —
#   테스트 본문 무변경, check.py 해시상수 전환·run.bat pip줄·README [1b]가 v4 패치 본체)
# ─────────────────────────────────────────────────────────────────────────────
...
import bot_trendstack_signal as TB                  # §8 040da0d2
import bot_sidewaydca_signal as SB                  # f758ef6d (Stg8 인증판)
...
def main():
    ...
    ts_bot = TB.TrendStackSignalBot(); ts_bot.on_init({})
    ...
    try:
        df7 = pd.DataFrame(ts_bot._h7, columns=['ts', 'open', 'high', 'low', 'close', 'volume']).set_index('ts')
        fresh = TB.TrendStackSignalBot(); fresh.on_init({})
    ...
    verdict = (f"VERDICT Stg14완전체(OI+ATR) | {'PASS' if ok else 'FAIL'} — 예외{err}·동치(oi포함){match}·"
