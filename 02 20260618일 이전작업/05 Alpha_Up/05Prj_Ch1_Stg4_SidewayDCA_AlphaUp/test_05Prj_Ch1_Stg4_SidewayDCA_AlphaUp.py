# -*- coding: utf-8 -*-
# [파일명] test_05Prj_Ch1_Stg4_SidewayDCA_AlphaUp.py
# 코드길이: 약 540줄 | 내부버전: SidewayDCA_AlphaUp_05_Ch1_Stg4 (정밀필터+sl_mult1.8) | 로직 축약/생략 없이 전체 출력
# ─────────────────────────────────────────────────────────────────────────────
# [이 코드가 하는 일 — 고딩 설명]
#   stg1~3(정직화·왕복비용·펀딩전체·손절깊이sl_mult)을 계승하고, stg4에서 두 가지를 추가한다.
...
#     ⑧ 정밀 진입필터   : stg4 데이터분석 결과 — regime_shift 장세에서 '변동압축(atr_ratio<0.9)'일 때 거래가 망함
...
#
# [PATH] 실행: D:\ML\verify\05Prj_Ch1_Stg4_SidewayDCA_AlphaUp\ . 데이터/펀딩: 상위 D:\ML\verify\ .
...
# ── stg4 설정: 펀딩 전체본 + sl_deep 손절깊이 스윕(캡 방식 폐기) ──
...
#   stg3 실데이터: 1.8이 WFE80.8%(최고)+MDD-13.4%(폭주선 통과). stg4는 인접 1.7/1.9로 '고원'인지 확인.
SL_MULT_GRID = [1.2, 1.5, 1.7, 1.8, 1.9, 2.2, 2.6]   # 1.7/1.9 추가 = 1.8 주변 고원 검증
DEFAULT_SLMULT = 1.8               # stg4 확정 기준선(stg3에서 1.5→1.8)

# ── stg4 정밀 진입필터 (요소1+4: regime_shift & 변동압축 차단) ──
#   stg4 데이터분석 결과: 위험은 '변동확대'가 아니라 '변동압축(atr_ratio<0.9)'이었고,
#   장세조건(regime_shift)과 결합해야 알짜(clean_range)를 안 죽인다.
#   atr_ratio = 데이터 컬럼(feat_, 그 시점 과거기반). regime_shift = 봇이 그 시점 계산하는 scen 라벨.
#   둘 다 진입 결정 시점에 알 수 있는 값 → 미래참조 없음.
FILTER_MODE      = 'precise'       # 'off'/'precise'(압축<lo 차단)/'both_ends'(압축<lo or 과열>=hi 차단)
ATR_LO           = 0.9             # 변동압축 경계(미만이면 위험)
ATR_HI           = 1.3             # 과열 경계(이상이면 위험, both_ends에서만)
FILTER_SCENS     = ('regime_shift',)  # 이 장세에서만 필터 적용(다른 장세 보존)
...
def load_1m(path):
    ...
    if has_vol:
        cols.append('volume')
    # stg4: 정밀필터용 atr_ratio(feat_ 컬럼) 있으면 함께 읽음. 없으면 필터 자동 비활성.
...
# ── 정직 엔진(1분봉 경로 청산 + 실펀딩 + sl_deep 손절깊이 sl_mult + stg4 진입필터) ──
def run_bot_honest(df, sig, par, mO, mH, mL, mT, ss, se, ft, fr, sl_mult,
    ...
    while j < n:
        ...
        if pos != 0:
            ...
            #     stg4 핵심: 손절깊이를 진입(dist_max)과 분리해 sl_mult로 조절(캡 방식 폐기).
        ...
        if not np.isnan(dev) and not np.isnan(A) and not strong and (j + 1 < n):
            if new_pl and dev < 0 and abs(dev) <= dist_max:
                ...
                if blocked(scen_now, atr_ratio[j]):          # stg4 정밀필터: 위험구간 진입 차단
                    blocked_n += 1
                else:
                    px = open_[j + 1]
...
def main():
    print("[SidewayDCA_AlphaUp_05_Ch1_Stg4] 정밀필터(regime&atr<0.9) + sl_mult 1.8 확정(1.7/1.9 인접확인)")
    ...
    # (E) stg4 핵심: 정밀필터 검증 스윕 (sl_mult=1.8 고정, 필터 모드 비교)
    blk_report = {}
    def run_variant(label, fmode, short_sz_mult=1.0):
        global SHORT_SIZE
        old_ss = SHORT_SIZE
        SHORT_SIZE = old_ss * short_sz_mult
        tr, _, _, blk = run_bot_honest(df8, sig, BEST_PAR, mO, mH, mL, mT, ss, se, ft, fr,
                                       DEFAULT_SLMULT, filter_mode=fmode,
        ...
        rows.append({'cell': label, 'cap_pct': DEFAULT_SLMULT, 'full_trades': av['trades'],
    ...
    if has_atrr:
        ...
        run_variant('FILTER_precise(regime&atr<0.9)', 'precise')
        run_variant('FILTER_both_ends(<0.9 or >=1.3)', 'both_ends')
        run_variant('FILTER_precise+short0.7x', 'precise', short_sz_mult=1.4)  # 0.5*1.4=0.7
    else:
    ...
    def findrow(name):
        for r in rows:
            if r['cell'] == name: return r
        return None
    filt_msg = "필터불가(atr_ratio없음)"
    if has_atrr:
        off = findrow('FILTER_off(slmult1.8)'); pre = findrow('FILTER_precise(regime&atr<0.9)')
        if off and pre:
            filt_msg = (f"OFF[PF{off['full_PF']} cumR{off['full_cumR']}% MDD{off['full_MDD']}% WFE{off['WFE']}%] "
                        f"-> precise[PF{pre['full_PF']} cumR{pre['full_cumR']}% MDD{pre['full_MDD']}% WFE{pre['WFE']}% "
                        f"차단{pre['sl_cap_hits']}건]")

    verdict = (f"VERDICT stg4 정밀필터+sl_mult1.8 | base(slmult1.8·필터OFF) PF={a_h0['PF']} cumR={a_h0['cumR_pct']}% "
               f"MDD={a_h0['MDD_pct']}% WFE={wfe_h0}% | 필터: {filt_msg} "
               f"| sl_mult추천(필터OFF)={cap_msg} | 펀딩={fund_note} | atr_ratio={'O' if has_atrr else 'X'}")
    ...
    # trades 저장용 거래원장: 필터 가동 시 precise(stg4 주력), 아니면 base(slmult1.8 필터OFF)
    if has_atrr:
    ...
    # trades = stg4 주력 거래원장(필터 가동시 precise)
    ...
    # scenarios = stg4 주력 거래의 8시나리오(train/test)
    ...
    # check.py 가 읽을 메트릭(인트라바 모호도 + stg4 필터 거래라벨)
