# -*- coding: utf-8 -*-
# [파일명] check_05Prj_Ch1_Stg7_SidewayDCA_AlphaUp.py
# 코드길이: 약 165줄 | 내부버전: SidewayDCA_AlphaUp_05_Ch1_Stg7 | 로직 전체 출력
...
VER = "SidewayDCA_AlphaUp_05_Ch1_Stg7"
TESTPY = "test_05Prj_Ch1_Stg7_SidewayDCA_AlphaUp.py"
CHECKPY = "check_05Prj_Ch1_Stg7_SidewayDCA_AlphaUp.py"
...
def check_all():
    ...
    # 10. stg7 정밀필터 작동 증빙 — summary에 FILTER 행이 있고 차단건수가 기록됐는지
    ...
    # 11. stg7 OI 2차필터 작동 증빙 — summary에 OI_ 행이 있는지(또는 불가 안전장치)
    ...
    # 12. stg7 측정 작동 증빙 — summary에 SL2_(차등sl) + TPTRACK_(익절추적) 행이 있는지
    s7_ok = False; s7_memo = "SL2/TPTRACK 행 없음"
    if os.path.exists(sp):
        try:
            s = pd.read_csv(sp)
            n_sl2 = s['cell'].astype(str).str.contains('SL2_regimeLong', na=False).sum()
            n_tp = s['cell'].astype(str).str.contains('TPTRACK_', na=False).sum()
            if n_sl2 > 0 and n_tp > 0:
                s7_ok = True; s7_memo = f"차등sl {n_sl2}변형 + 익절추적 {n_tp}구간 측정완료"
            elif n_sl2 > 0 or n_tp > 0:
                s7_ok = True; s7_memo = f"부분측정(SL2={n_sl2}, TPTRACK={n_tp}) — OI없어 일부 비활성 가능"
        except Exception as e:
            s7_memo = f"확인실패:{e}"
    res.append(("12.stg7측정작동", s7_ok, s7_memo))
