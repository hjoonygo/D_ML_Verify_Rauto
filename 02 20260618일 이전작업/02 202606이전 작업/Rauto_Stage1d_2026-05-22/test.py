# -*- coding: utf-8 -*-
# [파일명] test.py  (Stage1b)
# 코드길이: 약 320줄, 내부버전명: Stage1d_fibolock_sl, 로직 축약/생략 없이 전체 출력
#
...
#
# [★결과 전량 파일저장] S1d_trades_*.csv + S1d_summary.csv. 화면 복붙 불필요.
...
from Exec_Fibo_v3 import Exec_Fibo_v3   # ★Fibo_v2: 1차익절후 곧장피보 + 0.5 + 32bp하한선
...
CONFIGS = [
    ...
    (2.5, 1, 'rr25_sl1'),
    (2.5, 3, 'rr25_sl3'),
]  # (RR_MIN, sl_mode, tag). sl_mode 1=진입가-32bp, 3=원래OB SL
...
def simulate_one(exec_eng, df, o, h, l, c, idx, e_idx, gate, capital, sl_mode, split_ratio=0.5):
    ...
    notional = min(notional, capital * 2)  # 명목 클램프 자본×2(보수화, N2방지)
    ...
    bs = {'position': 'SHORT', 'entry_price': entry, 'remaining_pct': 1.0,
          ...
          'bullish_obs': [], 'bearish_obs': [], 'entry_regime': 'downtrend', 'entry_reason': 'gate', 'init_sl_price': gate['sl_price'],
          'lh_price': entry, 'floor_init': None, 'reduced_once': False}
    first_i = e_idx + 1
    w0 = max(0, first_i - 60 + 1)
    bs['df_1m'] = df.iloc[w0:first_i+1]
    params = {'leverage': LEVERAGE, 'fib_trigger_roe': 15.0, 'fib_sl_roe': 3.0, 'fib_ext_pct': 0.5,
              'innovation1': True, 'sl_mode': sl_mode}
    size = notional
    reduced = False
    pnl_total = 0.0
    ...
    for i in range(e_idx+1, end_idx):
        ...
        for price in ticks:
            ...
            if act == 'REDUCE_SHORT' and not reduced:
                ...
                # 본전스탑 제거: 엔진의 32bp 하한선이 잔량 방어 대체
                continue
            if act == 'CLOSE_SHORT':
...
def run_config(df, o, h, l, c, idx, down_idx, hp, lp, rr_min, sl_mode):
    ...
    while dptr < len(down_idx):
        ...
        if gate['pass']:
            gate_stats['pass'] += 1
            rows, pnl, why = simulate_one(exec_eng, df, o,h,l,c, idx, e_idx, gate, cap, sl_mode)
            cap += pnl
...
def main():
    print("[Stage1d] 게이트+검증청산엔진(다중OB계단)+복리위험7%+강제청산")
    ...
    summary = []
    for rr_min, sl_mode, tag in CONFIGS:
        trades, curve, bankrupt, gs = run_config(df, o,h,l,c, idx, down_idx, hp, lp, rr_min, sl_mode)
        pd.DataFrame(trades).to_csv(os.path.join(HERE, f"S1d_trades_{tag}.csv"), index=False, encoding='utf-8-sig')
        ...
        if len(trades):
            ...
            row = dict(config=tag, rr=rr_min, sl=f'안{sl_mode}',
            ...
            row = dict(config=tag, rr=rr_min, sl=f'안{sl_mode}', 진입수=0,
    ...
    pd.DataFrame(summary).to_csv(os.path.join(HERE, "S1d_summary.csv"), index=False, encoding='utf-8-sig')
    print("[저장] S1d_trades_*.csv (4) + S1d_summary.csv — 전량 파일저장 완료")
