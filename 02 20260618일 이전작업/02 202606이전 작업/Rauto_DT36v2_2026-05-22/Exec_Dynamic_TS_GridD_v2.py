...
class Exec_Dynamic_TS_GridD_v2:
    ...
    def check_exit(self, current_price: float, bot_state: dict, params: dict) -> dict:
        ...
        # ============================================================
        # [패치 v10b — 통합판_v2 문서 명시 수정] 진입 즉시 하드플로어 항상 ON (Phase 무관)
        #   원본 구멍: Phase1에서 첫 OB 도달 전 fib_stop=None -> 손절 부재 -> 역방향 90일 폭주.
        #   수정: 진입가 기준 hard_floor_roe(ROE %) 손절을 '모든 단계에서 상시' 검사.
        #   hard_floor_roe 미지정 시 fib_sl_roe로 폴백(기존과 동일 동작).
        # ============================================================
        hard_floor_pct = params.get('hard_floor_roe', params.get('fib_sl_roe', 3.0)) / 100.0
        if pos == "LONG":
            hf = entry * (1 - hard_floor_pct / leverage)
            if current_price <= hf:
                return {"action": "CLOSE_LONG", "reason": f"하드플로어 손절 (-{hard_floor_pct*100:.2f}% ROE)"}
        else:  # SHORT
            hf = entry * (1 + hard_floor_pct / leverage)
            if current_price >= hf:
                return {"action": "CLOSE_SHORT", "reason": f"하드플로어 손절 (-{hard_floor_pct*100:.2f}% ROE)"}
