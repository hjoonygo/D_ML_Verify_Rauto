...
class Exec_Fibo_v4:
    ...
    def check_exit(self, current_price: float, bot_state: dict, params: dict) -> dict:
        ...
        # ------------------------------------------------------------
            ...
            # [TP 제거판] 1차익절 없음. 진입 즉시 피보락인만으로 추세 끝까지 태움.
            #   신저점(LL) 갱신마다 직전 눌림고점(LH)~새저점(LL)의 fib_ext(0.382) 지점으로 손절선 끌어내림.
            #   첫 보호선 = SL 3안(여기선 3안=원래 OB SL). 락인이 floor보다 유리해지면 락인이 청산 관리.
            sl_mode = params.get('sl_mode', 3)
            if bot_state.get('floor_init') is None:
                if sl_mode == 1:
                    bot_state['floor_init'] = entry * (1 - 0.0032)
                else:
            ...
            if current_price >= stop_line:
                tag = 'Fibonacci 0.382 락인' if bot_state.get('fib_stop') is not None and bot_state['fib_stop'] < floor else 'SL보호선'
