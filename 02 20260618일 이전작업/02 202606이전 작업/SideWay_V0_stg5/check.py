# [check.py]
# 코드길이: 197줄 | 내부버전명: SideWay_V0_stg5
# 로직 축약/생략 없이 전체 출력. 아래 In/Out 태그 후 코드 시작.
# =====================================================================================
# 목적(stg5): backtest.py 산출물(108셀)을 8개 시나리오로 검증, 전량 파일 기록.
#       시나리오5(시가체결)는 거래의 tf_h별 시가맵으로 매칭. 결과/분석txt는 실행 하위폴더,
#       INDEX만 ../00WorkHstr 누적. 분석txt에 상위셀(수익순)·비용드래그·레짐손익 기재.
# === 파일 In/Out ===
#  In : ./SideWay_V0_stg5_manifest.json/_results.json/_summary.csv/_trades.csv , ../merged_data.csv
...
# === 핵심 변수: TOL(가격정합), checks(8), omap_by_h(시간별 봉시가맵)
# =====================================================================================
...
def open_map_tf(df1m, hrs):
    opens = df1m.resample(f"{hrs}h", origin="epoch", label="left", closed="left")["open"].first().dropna()
...
def run_8_scenarios(parent, out_dir, v, manifest, results, summary, trades):
    ...
    # 5: 시가체결 — tf_h별 시가맵
    hours = sorted(set(int(h) for h in trades["tf_h"].unique())) if len(trades) else []
    omap = {h: open_map_tf(df, h) for h in hours}
    e_bad = x_bad = checked = 0
    for _, tr in trades.iterrows():
        mp = omap.get(int(tr["tf_h"]), {})
    ...
    checks.append(("7.자본 정합성(복리)", len(recon_bad) == 0 and neg == 0,
                   f"복리재현 {'일치' if not recon_bad else '불일치 '+str(len(recon_bad))+'셀'} / 음수자본 {neg}"))
...
def write_report(out_dir, checks, all_pass, results, summary):
    ...
    lines = [f"# 작업분석 보고 — {VERSION} (TF4~12h × dropRate1.0~0.5 × 사이즈1.0/2.5)",
    ...
    cols = ["tf_h", "drop_rate", "frac", "native", "num_trades", "total_return_pct", "CAGR_pct",
            "MDD_pct", "sharpe", "win_rate_pct", "cost_drag_pct", "regime_down_pnl"]
    cols = [c for c in cols if c in summary.columns]
    lines += ["", "== 수익 상위 20셀 =="]
    lines.append("  " + " | ".join(cols))
    for _, r in summary.sort_values("total_return_pct", ascending=False).head(20).iterrows():
        lines.append("  " + " | ".join(str(r[c]) for c in cols))
    lines += ["", "== native(4/6/8/12h) 한정 사이즈1.0배 상위 =="]
    nat = summary[(summary["native"] == True) & (summary["frac"] == 1.0)].sort_values("total_return_pct", ascending=False).head(10)
    for _, r in nat.iterrows():
        lines.append("  " + " | ".join(str(r[c]) for c in cols))
    lines += ["", "== 판정 메모 ==",
              "  dropRate↓ 시 거래수↑면서 승률/PF/샤프가 버티면 빈도 엔진 확보. 무너지면 빈도 천장.",
              "  실거래는 native TF(4/6/8/12h) 우선. 5/7/9/10/11h는 연구용(거래소 기본봉 아님).",
              "  사이즈 2.5배는 1.0배 대비 수익·MDD 비례 확대(샤프 유지). 직접데이터, 신뢰도 ~85%."]
...
def append_index(index_dir, now, all_pass, checks, results, summary):
    ...
    b = summary.sort_values("total_return_pct", ascending=False).iloc[0]
    line = (f"{now} | {VERSION} | {'PASS' if all_pass else 'FAIL'} {npass}/{len(checks)} | "
            f"best=tf{b['tf_h']}h/dr{b['drop_rate']}/x{b['frac']} trades={b['num_trades']} ret={b['total_return_pct']}% "
            f"CAGR={b['CAGR_pct']}% MDD={b['MDD_pct']}% sharpe={b['sharpe']} | data={q['start'][:10]}~{q['end'][:10]}")
