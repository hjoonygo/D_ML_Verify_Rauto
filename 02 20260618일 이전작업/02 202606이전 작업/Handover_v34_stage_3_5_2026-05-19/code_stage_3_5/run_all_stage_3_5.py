# -*- coding: utf-8 -*-
"""
[파일명] run_all_stage_3.py
[코드길이] 약 280줄
[목적] Stage 3.5 측정 일괄 실행 — 사용자 PC entry point
...
[흐름]
  ...
  4. 모델 backup (재학습 시)
  5. 학습 (--skip-train 가능)
  6. 단위 테스트 (test_v11_stage_3_5.py)
  7. 측정 (measure_v34_stage_3_5.py)
  8. 결과 안내
...
def run_unit_tests():
    print_step(5, 7, "Unit tests (v11 Stage 3.5, ~30 sec)")
    ok, elapsed = _run_subprocess("test_v11_stage_3_5.py", "Unit tests")
    if not ok:
        return False
...
def run_measurement():
    print_step(6, 7, "Measurement (4 scenarios — SL_MAX grid, expected 5-15 min with cache)")
    ok, elapsed = _run_subprocess("measure_v34_stage_3_5.py", "Measurement")
    if not ok:
        return False
...
def show_completion():
    ...
    print("\n  Next: zip the entire 'outputs_stage_3_5' folder and upload.")
...
def main():
    ...
    print_header("Rauto V34 Stage 3.5 - Regime Policy")
    print("  Working dir: " + WORK_DIR)
    print("  Grid: 4 scenarios (regime policy 4 시나리오 bp at OB TF 60m)")
    print("  Rules: uptrend long block + hivol_range long SL500/16H 또는 18H")
