...
# ★ V80k_Verify_2: Strategy ZIP 내부 models/ 우선 + 기존 fallback 유지
_STRATEGY_DIR = os.path.dirname(os.path.abspath(__file__))
_STRATEGY_MODELS_DIR = os.path.join(_STRATEGY_DIR, 'models')
BASE_DIR = os.environ.get('PAUTO_BASE_DIR') or _STRATEGY_DIR

# 1순위: strategy 자기 폴더의 models/ (ZIP 추출 시 자동 위치)
if os.path.exists(os.path.join(_STRATEGY_MODELS_DIR, "PautoV80_Regime_Model_v6.json")):
    BASE_DIR = _STRATEGY_MODELS_DIR
# 2순위: BASE_DIR 직접
elif not os.path.exists(os.path.join(BASE_DIR, "PautoV80_Regime_Model_v6.json")):
    # 3순위: 기존 fallback
