...
class DynamicComboBox(QComboBox):
    ...
    def populate_items(self):
        ...
        self.clear()
        # ★ V80k_Verify_2: Default "선택 안 함" — 사용자 명시적 선택 강제
        files = ["— 선택 안 함 —"]
        ...
        if current in files:
            ...
            self.setCurrentIndex(0)  # default = 선택 안 함


class StrategyComboBox(QComboBox):
    """★ V80k_Verify_2: 전략 ZIP 자동 스캔.
    
    strategies/*.zip 자동 감지 → 메타데이터 표시.
    선택 시 BotManager.update_strategy() 호출.
    """
    def __init__(self):
        super().__init__()
    ...
    def populate_items(self):
        ...
        # 시작 default
        items = ["— 전략 선택 안 함 —"]
        self._strategies_info = [{'name': None}]
        
        try:
            from StrategyLoader import list_strategies
            strats = list_strategies()
            for s in strats:
                marker = '★ Observer' if s['is_observer'] else '거래'
                label = f"{s['name']} [{marker}] ({s['size_mb']:.1f}MB)"
                items.append(label)
                self._strategies_info.append(s)
        except Exception as e:
            items.append(f"(스캔 실패: {e})")
    ...
    def get_selected_strategy(self):
        """선택된 전략 메타정보. None이면 '선택 안 함'."""
        idx = self.currentIndex()
        if idx <= 0 or idx >= len(self._strategies_info):
            return None
        return self._strategies_info[idx]
        self.addItems(files)
