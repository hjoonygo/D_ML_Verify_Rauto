...
def run_unit_tests():
    print_step(5, 7, "Unit tests (8 tests, about 3 min)")
    ok, elapsed = _run_subprocess("test_v7_phase_a.py", "Unit tests")
    if not ok:
        return False
...
def run_measurement():
    print_step(6, 7, "Measurement (12 scenarios, expected 1-3 hours)")
    ok, elapsed = _run_subprocess("measure_v34_phase_b.py", "Measurement")
    if not ok:
        return False
...
def show_completion():
    ...
    print("\n  Next: zip the entire 'outputs_phase_b' folder and upload.")
...
def main():
    ...
    print_header("Rauto V34 Phase B - Plan A (Dynamic Hard SL) + Plan D (Vol Filter)")
    print("  Working dir: " + WORK_DIR)
    print("  Grid: 12 scenarios (ATR multi 4 x Lev 1 x Filter 3)")
