# -*- coding: utf-8 -*-
# [FILE] check.py  (SpTrd_Fib_V0_5 - contamination 8-check + verdict + INDEX)
# CODE LENGTH: approx 190 lines | INTERNAL VER: check_spfib_v05 | full output, no omission
#
# [PURPOSE] test.py 직후. (1)오염검사 8항목 (2)분석txt(7h가 6/8h와 함께 양인지·알파판정)
#   (3)INDEX 1줄. 출력 -> 상위 D:\ML\verify\00WorkHstr\. 전량 파일(복붙요청 없음).
#
# [8 SCENARIOS]
#   1 결과파일존재 : spfib_summary / spfib_trades ?
#   2 잔존섞임없음 : mtime > .run_start ?
#   3 파일명일치   : spfib_*.csv 허용패턴만?
#   4 데이터정상   : 상위데이터 행수/기간/해시 +36mo경고
#   5 중복없음     : spfib_trades 같은 진입시간 중복 0?
...
SUMMARY = "spfib_summary.csv"; TRADES = "spfib_trades.csv"
...
def run_checks(start_ts):
    ...
    found = [os.path.basename(f) for f in glob.glob(os.path.join(HERE, "spfib_*.csv"))]
...
def write_analysis(checks, analysis):
    ...
    if os.path.exists(sp):
        try:
            ...
            c1 = getrow('F0_nofilter'); c2 = getrow('F1_short_vguard'); c3 = getrow('F2_short_strong')
            if c1 is not None:
            ...
            ct = getrow('F1_train'); ce = getrow('F1_test')
            ...
            cl = getrow('F3_long_only_bear'); cs = getrow('F0_nofilter')
