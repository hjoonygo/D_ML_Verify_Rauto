# -*- coding: utf-8 -*-
# [FILE] check.py  (SpTrd_Fib_V0_stg8 - contamination 8-check + verdict + INDEX)
# CODE LENGTH: approx 190 lines | INTERNAL VER: check_sfstg8 | full output, no omission
#
...
SUMMARY = "sfstg8_summary.csv"; TRADES = "sfstg8_trades.csv"
...
def run_checks(start_ts):
    ...
    found = [os.path.basename(f) for f in glob.glob(os.path.join(HERE, "sfstg8_*.csv"))]
    ...
    if os.path.exists(paths[SUMMARY]):
        try:
            ...
            if '칸' in s.columns:
                mask = ~s['칸'].astype(str).str.startswith('VERDICT')
                if '거래수' in s.columns:
...
def write_analysis(checks, analysis):
    ...
    if os.path.exists(sp):
        try:
            s = pd.read_csv(sp)
            # VERDICT 메모행
            vrow = s[s['칸'].astype(str).str.startswith('VERDICT')]
            if len(vrow):
                verdict_lines.append(str(vrow.iloc[0]['칸']))
            def te(lab):
                r = s[s['칸'].astype(str) == lab + '_test']
                return pd.to_numeric(r.iloc[0]['PF'], errors='coerce') if len(r) else None
            def al(lab):
                r = s[s['칸'].astype(str) == lab + '_all']
                return r.iloc[0] if len(r) else None
            # 단독 vs 조합 test PF
            c1 = te('C1_adx20'); c2 = te('C2_atrcmp'); c3 = te('C3_OR'); c4 = te('C4_AND')
            verdict_lines.append(f"단독 test PF: adx20={c1} atrcmp={c2} | 조합: OR={c3} AND={c4}")
            solo = max([x for x in [c1, c2] if x is not None] or [0])
            combo = max([x for x in [c3, c4] if x is not None] or [0])
            verdict_lines.append(f"-> {'조합 시너지 있음(조합>단독)' if combo > solo else '조합 무익(단독이 낫거나 같음)'}")
            # S4 자본곡선(각 칸 all행에 S4_최종/MDD)
            for lab in ['C1_adx20', 'C3_OR', 'C4_AND']:
                r = al(lab)
                if r is not None and 'S4_최종' in r:
                    verdict_lines.append(f"  {lab}: S4최종 {r['S4_최종']} MDD {r['S4_MDD_pct']}% 파산 {r.get('S4_파산','?')}")
        except Exception as e:
