# -*- coding: utf-8 -*-
# [파일명] test.py  (Stage1b)
# 코드길이: 약 320줄, 내부버전명: Stage1e_fibo0382_rr5, 로직 축약/생략 없이 전체 출력
#
...
#
# [★결과 전량 파일저장] S1e_trades_*.csv + S1e_summary.csv. 화면 복붙 불필요.
...
CONFIGS = [
    ...
    (4.5, 3, 'rr45'),
    (5.0, 3, 'rr50'),
]  # SL 3안(원래OB SL) 고정. RR_MIN 3.0~5.0. 락인 0.382.
...
def main():
    print("[Stage1e] 게이트+검증청산엔진(다중OB계단)+복리위험7%+강제청산")
    ...
    for rr_min, sl_mode, tag in CONFIGS:
        ...
        pd.DataFrame(trades).to_csv(os.path.join(HERE, f"S1e_trades_{tag}.csv"), index=False, encoding='utf-8-sig')
    ...
    pd.DataFrame(summary).to_csv(os.path.join(HERE, "S1e_summary.csv"), index=False, encoding='utf-8-sig')
    print("[저장] S1e_trades_*.csv (4) + S1e_summary.csv — 전량 파일저장 완료")
