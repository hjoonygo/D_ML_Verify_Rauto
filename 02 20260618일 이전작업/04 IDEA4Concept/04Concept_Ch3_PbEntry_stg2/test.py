...
def exec_check_exit(price, bs, params):
    ...
    #   보호선 가격 hf = 진입가에서 불리방향 +3%(=hard_floor_roe/lev). 숏:위 / 롱:아래.
    if bs['fib_stop'] is None:
        hf = entry * (1 - DIR * params['hard_floor_roe'] / 100.0 / lev)
        if DIR * (hf - price) >= 0:          # 숏:price>=hf / 롱:price<=hf
            return {"action": "CLOSE", "reason": "hole_hardfloor"}
    # (2) 빈구간 보호선: REDUCE 후 & 피보 미발동. g=불리방향 +3% 가격, tight면 last_ob_top과 더 가까운 쪽.
    gm = params['guard_mode']
    if gm != 'off' and bs['reduced'] and not bs['fib_active']:
        g = entry * (1 - DIR * HARD_FLOOR_ROE / 100.0 / lev)
        if gm == 'tight' and bs['last_ob_top'] is not None:
            # 진입가에 더 가까운(=더 타이트한 보호) 쪽. 숏:더 낮은 g / 롱:더 높은 g
            g = min(g, bs['last_ob_top']) if DIR < 0 else max(g, bs['last_ob_top'])
        if DIR * (g - price) >= 0:            # 숏:price>=g / 롱:price<=g
            return {"action": "CLOSE", "reason": "gap_guard"}
    bull = bs['bullish_obs']
    if ti < len(bull):
        tob = bull[ti]
        if DIR * (price - tob['mean']) >= 0:   # 숏:price<=mean / 롱:price>=mean (목표 유리통과)
        ...
        if bs['fib_stop'] is not None and DIR * (bs['fib_stop'] - price) >= 0:  # 숏:price>=stop / 롱:price<=stop
        ...
        if DIR * (price - bs['fib_extreme']) > 0:   # 숏:price<ext / 롱:price>ext (더 유리한 극값 갱신)
            if bs['pulled_back']:
        ...
        if roe >= params['fib_trigger_roe'] or max_roe >= params['fib_trigger_roe']:
            ...
            bs['fib_active'] = True
            if DIR * (bs['fib_stop'] - price) >= 0:   # 숏:price>=stop / 롱:price<=stop
                return {"action": "CLOSE", "reason": "Fibonacci"}
    return {"action": "HOLD", "reason": "hold"}


def adverse_to_roe(DIR, entry, price, lev, favor=True):
    """유리방향 수익률 ROE%. 숏:(entry-price)/entry, 롱:(price-entry)/entry. 일반화 DIR*(price-entry)."""
    return (DIR * (price - entry) / entry) * lev * 100.0
