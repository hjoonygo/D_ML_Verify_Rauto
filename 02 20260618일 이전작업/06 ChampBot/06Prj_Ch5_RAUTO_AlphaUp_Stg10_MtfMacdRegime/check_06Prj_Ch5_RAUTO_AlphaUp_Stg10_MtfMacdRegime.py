# -*- coding: utf-8 -*-
# [파일명] check_06Prj_Ch5_RAUTO_AlphaUp_Stg10_MtfMacdRegime.py
# 코드길이: 약 140줄 | 내부버전: RAUTO_AlphaUp_06_Ch5_Stg10_MtfMacdRegime | 전체 출력
# [검사 10항목] 1.필수파일 2.CSV非공백 3.코드해시 4.중복없음 5.미래참조가드+label제외+임계학습기간만
#   6.MACD장세OOS정확도 7.혼동행렬 8.3자비교(base/무조건/조건부) 9.다중TF+엔진해시 10.VERDICT
...
VER = "RAUTO_AlphaUp_06_Ch5_Stg10_MtfMacdRegime"
TESTPY = "test_06Prj_Ch5_RAUTO_AlphaUp_Stg10_MtfMacdRegime.py"
CHECKPY = "check_06Prj_Ch5_RAUTO_AlphaUp_Stg10_MtfMacdRegime.py"
...
REQ = [TESTPY, CHECKPY, "run.bat", TE, SE, "summary.csv", "macd_regime_oos.csv", "regime_confusion.csv", "pov_compare.csv", "pov_compare_by_regime.csv"]
...
def metric():
    p = os.path.join(HERE, ".stg10_metric"); d = {}
    if os.path.exists(p):
...
def run():
    ...
    dup = 0
    if pr.get("pov_compare.csv"):
        try:
            dup = int(pd.read_csv(os.path.join(HERE, "pov_compare.csv")).duplicated().sum())
        except Exception:
            dup = -1
    res.append(("4.중복없음", dup == 0, f"중복{dup}"))
    look = True; memo = "음수shift 미사용 — MACD/임계 과거봉·학습기간만"
    ...
    # 임계가 학습기간(:cut)만 쓰는지 코드 확인
    if "np.nanquantile(np.abs(m2[:cut" not in src:
        memo += " | (주의)임계학습기간 확인필요"
    else:
        memo += " | 임계=학습기간만 확인"
    res.append(("5.미래참조가드+임계", look, memo))
    res.append(("6.MACD장세OOS정확도", 'macd_acc_7h' in m, f"7h정확도{m.get('macd_acc_7h')}%(기준선{m.get('maj')}%)"))
    cok = pr.get("regime_confusion.csv") and os.path.getsize(os.path.join(HERE, "regime_confusion.csv")) > 20
    res.append(("7.혼동행렬", cok, "산출" if cok else "없음"))
    res.append(("8.3자비교", 'oos_ret_cond' in m, f"OOS base{m.get('oos_ret_base')}/무조건{m.get('oos_ret_uncond')}/조건부{m.get('oos_ret_cond')}%"))
    eng = True; em = []
    ...
    res.append(("9.다중TF+엔진해시", ('trend_auc' in m) and eng, f"AUC={m.get('trend_auc','')[:30]} | " + " ".join(em)))
...
def analysis(passed, res, hs, m):
    ...
    L += ["-"*60,
          f"[MACD장세] 7h정확도 {m.get('macd_acc_7h')}%(기준선{m.get('maj')}%) | 다중TF {m.get('macd_acc_all')} | 추세AUC {m.get('trend_auc')}",
          f"[OOS 3자] base PF{m.get('oos_pf_base')}/{m.get('oos_ret_base')}% | 무조건 PF{m.get('oos_pf_uncond')}/{m.get('oos_ret_uncond')}% | 조건부 PF{m.get('oos_pf_cond')}/{m.get('oos_ret_cond')}%",
          f"[전체 3자] base {m.get('full_ret_base')}% | 무조건 {m.get('full_ret_uncond')}% | 조건부 {m.get('full_ret_cond')}%",
