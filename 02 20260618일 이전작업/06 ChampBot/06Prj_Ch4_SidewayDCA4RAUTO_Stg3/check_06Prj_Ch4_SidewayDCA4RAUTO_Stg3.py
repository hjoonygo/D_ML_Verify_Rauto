# -*- coding: utf-8 -*-
# [파일명] check_06Prj_Ch4_SidewayDCA4RAUTO_Stg3.py
# 코드길이: 약 200줄 | 내부버전: ChampBot_SidewayDCA4RAUTO_06_Ch4_Stg3 | 로직 전체 출력
# [역할] test.py 후: (1)오염검사 8항목 (2)분석txt를 상위 00WorkHstr 저장 (3)INDEX 한줄. 전량 파일로만(복붙 금지).
# [입력] sfrs_summary.csv / sfrs_trades.csv / sfrs_equity.csv / .sfrs_metric (실행폴더)
...
VER = "ChampBot_SidewayDCA4RAUTO_06_Ch4_Stg3"
TESTPY = "test_06Prj_Ch4_SidewayDCA4RAUTO_Stg3.py"
CHECKPY = "check_06Prj_Ch4_SidewayDCA4RAUTO_Stg3.py"
REQUIRED = [TESTPY, CHECKPY, "run.bat", "sfrs_summary.csv", "sfrs_trades.csv", "sfrs_equity.csv"]
...
def parse_verdict():
    sp = os.path.join(HERE, "sfrs_summary.csv")
...
def read_metric():
    p = os.path.join(HERE, ".sfrs_metric")
    d = {}
    if os.path.exists(p):
...
def check_all():
    ...
    csvs = ["sfrs_summary.csv", "sfrs_trades.csv", "sfrs_equity.csv"]
    ...
    dup = 0; tp = os.path.join(HERE, "sfrs_trades.csv")
    if present.get("sfrs_trades.csv"):
        try:
            t = pd.read_csv(tp)
    ...
    off_n = int(metric.get("off_n", 0) or 0); on_n = int(metric.get("on_n", 0) or 0)
    if has_oi:
        ok6 = on_n <= off_n   # 무덤차단이므로 ON 거래수 <= OFF
        memo6 = f"거래 OFF{off_n}->ON{on_n} (ON<=OFF여야 정상)"
    else:
        ok6 = True; memo6 = "OI없음 → 필터OFF만(상위 D:\\ML\\verify\\Merged_Data.csv 확인)"
    res.append(("6.OFFvsON정상", ok6, memo6))

    scen_ok = False; sp = os.path.join(HERE, "sfrs_summary.csv")
    if present.get("sfrs_summary.csv"):
        try:
