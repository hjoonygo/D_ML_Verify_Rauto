# -*- coding: utf-8 -*-
# [파일명] check_06Prj_Ch4_SidewayDCA4RAUTO_Stg7.py
# 코드길이: 약 200줄 | 내부버전: ChampBot_SidewayDCA4RAUTO_06_Ch4_Stg7 | 로직 전체 출력
# [역할] test.py 후: (1)오염검사 8항목 (2)분석txt를 상위 00WorkHstr 저장 (3)INDEX 한줄. 전량 파일로만(복붙 금지).
# [입력] sd_summary.csv / sd_trades.csv / sd_yearly.csv / .sd_metric (실행폴더)
...
VER = "ChampBot_SidewayDCA4RAUTO_06_Ch4_Stg7"
TESTPY = "test_06Prj_Ch4_SidewayDCA4RAUTO_Stg7.py"
CHECKPY = "check_06Prj_Ch4_SidewayDCA4RAUTO_Stg7.py"
REQUIRED = [TESTPY, CHECKPY, "run.bat", "sd_summary.csv", "sd_trades.csv", "sd_yearly.csv"]
...
def parse_verdict():
    sp = os.path.join(HERE, "sd_summary.csv")
...
def check_all():
    ...
    csvs = ["sd_summary.csv", "sd_trades.csv", "sd_yearly.csv"]
    ...
    dup = 0; tp = os.path.join(HERE, "sd_trades.csv")
    if present.get("sd_trades.csv"):
        try:
            t = pd.read_csv(tp)
    ...
    look = True; memo = "음수shift 없음 + oi_zscore resample last(과거24h, 미래참조없음)"
    if present.get(TESTPY):
        ...
        elif "oi_mode" not in code:
            look = False; memo = "oi_mode 무덤구간 분기 없음-수동확인"
        elif "deadzone" not in code:
            look = False; memo = "deadzone 모드 없음-수동확인"
    else:
    ...
    if has_oi:
        # Stg7: 연도별 3모드(off/hi/dz) 매트릭스(sd_yearly)에 행이 채워졌는지
        mlp = os.path.join(HERE, "sd_yearly.csv")
        ok6 = False; memo6 = "sd_yearly 없음"
        if os.path.exists(mlp):
            try:
                mj = pd.read_csv(mlp)
                ok6 = ("dz_cumR" in mj.columns) and ("hi_cumR" in mj.columns) and (len(mj) >= 3)
                memo6 = f"연도별 3모드 {len(mj)}행 (off/hi/dz)"
            except Exception as e:
                memo6 = f"읽기실패:{e}"
    else:
        ok6 = True; memo6 = "OI없음 → 검증불가(상위 D:\\ML\\verify\\Merged_Data.csv 확인)"
    res.append(("6.연도3모드기록", ok6, memo6))

    scen_ok = False; sp = os.path.join(HERE, "sd_summary.csv")
    if present.get("sd_summary.csv"):
        try:
            cells = pd.read_csv(sp)["cell"].astype(str)
            # B 구조: S1(3모드)·S6(순열)·S7(임계)·S8(위험)이 summary에, S2(연도)는 sd_yearly에 분산
            core = sum(1 for k in ["S1_", "S6_", "S7_", "S8_"] if cells.str.contains(k, na=False).any())
            yok = os.path.exists(os.path.join(HERE, "sd_yearly.csv"))
            verdict_ok = cells.str.contains("VERDICT", na=False).any()
            # 핵심 4시나리오 + 연도별파일 + verdict = 8요소 충족
            scen_ok = (core == 4) and yok and verdict_ok if has_oi else True
            memo7 = f"핵심 S1/S6/S7/S8 중 {core}개 + yearly{'O' if yok else 'X'} + verdict{'O' if verdict_ok else 'X'}"
        except Exception as e:
            memo7 = f"확인실패:{e}"
    else:
        memo7 = "summary없음"
    res.append(("7.시나리오완전", scen_ok, memo7))
