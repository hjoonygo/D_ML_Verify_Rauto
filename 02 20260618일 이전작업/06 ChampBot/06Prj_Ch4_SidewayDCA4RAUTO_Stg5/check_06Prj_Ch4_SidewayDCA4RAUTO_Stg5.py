# -*- coding: utf-8 -*-
# [파일명] check_06Prj_Ch4_SidewayDCA4RAUTO_Stg5.py
# 코드길이: 약 200줄 | 내부버전: ChampBot_SidewayDCA4RAUTO_06_Ch4_Stg5 | 로직 전체 출력
# [역할] test.py 후: (1)오염검사 8항목 (2)분석txt를 상위 00WorkHstr 저장 (3)INDEX 한줄. 전량 파일로만(복붙 금지).
# [입력] er_summary.csv / er_trades.csv / er_yearly.csv / .er_metric (실행폴더)
...
VER = "ChampBot_SidewayDCA4RAUTO_06_Ch4_Stg5"
TESTPY = "test_06Prj_Ch4_SidewayDCA4RAUTO_Stg5.py"
CHECKPY = "check_06Prj_Ch4_SidewayDCA4RAUTO_Stg5.py"
REQUIRED = [TESTPY, CHECKPY, "run.bat", "er_summary.csv", "er_trades.csv", "er_yearly.csv"]
...
def parse_verdict():
    sp = os.path.join(HERE, "er_summary.csv")
...
def check_all():
    ...
    csvs = ["er_summary.csv", "er_trades.csv", "er_yearly.csv"]
    ...
    dup = 0; tp = os.path.join(HERE, "er_trades.csv")
    if present.get("er_trades.csv"):
        try:
            t = pd.read_csv(tp)
    ...
    if has_oi:
        # Stg5: 연도별 일관성 매트릭스(er_yearly)에 연도×ER 행이 채워졌는지
        mlp = os.path.join(HERE, "er_yearly.csv")
        ok6 = False; memo6 = "er_yearly 없음"
        if os.path.exists(mlp):
            try:
                mj = pd.read_csv(mlp)
                ok6 = ("better" in mj.columns) and (len(mj) >= 8)
    ...
    scen_ok = False; sp = os.path.join(HERE, "er_summary.csv")
    if present.get("er_summary.csv"):
        try:
