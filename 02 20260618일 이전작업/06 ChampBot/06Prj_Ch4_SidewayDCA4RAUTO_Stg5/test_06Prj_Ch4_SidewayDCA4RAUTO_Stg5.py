# -*- coding: utf-8 -*-
# [파일명] test_06Prj_Ch4_SidewayDCA4RAUTO_Stg5.py
# 코드길이: 약 640줄 | 내부버전: ChampBot_SidewayDCA4RAUTO_06_Ch4_Stg5 | 로직 축약/생략 없이 전체 출력
# ─────────────────────────────────────────────────────────────────────────────
...
def main():
    print("[ChampBot_SidewayDCA4RAUTO_06_Ch4_Stg5] A-심화: ER 게이트 우연vs실력 확정")
    ...
    if not has_oi:
        for fn in ["er_summary.csv", "er_yearly.csv", "er_trades.csv"]:
            pd.DataFrame([{'cell': '★검증불가: oi_zscore 없음(상위 Merged_Data.csv 확인)'}]).to_csv(
                os.path.join(HERE, fn), index=False, encoding='utf-8-sig')
        with open(os.path.join(HERE, ".er_metric"), 'w', encoding='utf-8') as f:
            f.write("has_oi=0\n")
        print("[save] ★OI없음 - 검증불가"); return
    oi_arr = load_oi_8h(oipath, df_tf.index)
    bb_arr = load_bb_8h(data, df_tf.index)

    YEARS = [2023, 2024, 2025, 2026]
    ER_GRID = [0.35, 0.40, 0.45, 0.50, 0.55, 0.60]

    def run_er(ge):
        return run_strategy(df_tf, sig, 0, 'none', 0.8, dz_oi=oi_arr, gate_mode='er', gate_er=ge, gate_bb=bb_arr)

    # 원본·S2(무덤항상) 기준
    tr_off = run_strategy(df_tf, sig, 0, 'none', 0.8, dz_oi=None)
    tr_s2 = run_strategy(df_tf, sig, 0, 'none', 0.8, dz_oi=oi_arr, gate_mode='none')
    off_all = stats_of(tr_off); s2_all = stats_of(tr_s2)
    off_year = {y: stats_of(tr_off, [y]) for y in YEARS}

    rows = []; yearly = []

    # ── 시나리오1: ER 문턱 정밀 스윕 (전체 PF·수익금·MDD) ──
    er_results = {}
    for ge in ER_GRID:
        tr = run_er(ge); er_results[ge] = tr
        a = stats_of(tr)
        trn = stats_of(tr, [2023, 2024, 2025]); te = stats_of(tr, [2026])
        rows.append({'cell': f'S1_ER{ge}', 'n': a['n'], 'PF': a['PF'], 'cumR': a['cumR'],
                     'mdd': a['mdd'], 'fin': a['fin'], 'train_PF': trn['PF'], 'test_PF': te['PF'],
                     'gap': round(abs(trn['PF'] - te['PF']), 3), 'flips': a['flips'], 'note': ''})

    # ── 시나리오2: 연도별 일관성 (각 ER이 매년 원본보다 나은가) ──
    for ge in ER_GRID:
        tr = er_results[ge]
        better_years = 0; note = ""
        for y in YEARS:
            ys = stats_of(tr, [y]); os_ = off_year[y]
            win = ys['cumR'] >= os_['cumR']
            better_years += int(win)
            note += f"{y}:{ys['cumR']:+.0f}vs{os_['cumR']:+.0f}{'O' if win else 'X'} "
            yearly.append({'cell': f'ER{ge}_{y}', 'er': ge, 'year': y,
                           'er_cumR': ys['cumR'], 'off_cumR': os_['cumR'],
                           'er_PF': ys['PF'], 'off_PF': os_['PF'], 'better': int(win)})
        rows.append({'cell': f'S2_연도일관_ER{ge}', 'n': better_years, 'PF': '', 'cumR': '',
                     'mdd': '', 'fin': '', 'train_PF': '', 'test_PF': '', 'gap': '',
                     'flips': '', 'note': f"원본대비 우위 {better_years}/4년 | {note}"})

    # ── 시나리오3: ER vs S2 vs 원본 연도별 3자 ──
    for y in YEARS:
        e045 = stats_of(er_results[0.45], [y])
        rows.append({'cell': f'S3_3자_{y}', 'n': '', 'PF': '', 'cumR': '', 'mdd': '', 'fin': '',
                     'train_PF': '', 'test_PF': '', 'gap': '', 'flips': '',
                     'note': f"원본 {off_year[y]['cumR']:+.0f}% | S2 {stats_of(tr_s2,[y])['cumR']:+.0f}% | ER0.45 {e045['cumR']:+.0f}%"})

    # ── 시나리오4: gap 분해 (train 어느 해가 약했나, ER0.45) ──
    tr45 = er_results[0.45]
    g4 = ""
    for y in [2023, 2024, 2025]:
        ys = stats_of(tr45, [y]); g4 += f"{y}:PF{ys['PF']} "
    rows.append({'cell': 'S4_gap분해_ER0.45', 'n': '', 'PF': '', 'cumR': '', 'mdd': '', 'fin': '',
                 'train_PF': stats_of(tr45,[2023,2024,2025])['PF'], 'test_PF': stats_of(tr45,[2026])['PF'],
                 'gap': '', 'flips': '', 'note': f"train내부 {g4}| test2026 PF{stats_of(tr45,[2026])['PF']}"})

    # ── 시나리오5: 워크포워드 (과거고르고 미래검증) ──
    wf = [("23→24", [2023], [2024]), ("23~24→25", [2023, 2024], [2025]), ("23~25→26", [2023, 2024, 2025], [2026])]
    for nm, tr_y, te_y in wf:
        # train구간서 ER최적 고르고 test구간 성적
        best_ge = None; best_tr = -1
        for ge in ER_GRID:
            v = stats_of(er_results[ge], tr_y)['PF']
            if v > best_tr:
                best_tr = v; best_ge = ge
        te_pf = stats_of(er_results[best_ge], te_y)['PF']
        off_te = stats_of(tr_off, te_y)['PF']
        rows.append({'cell': f'S5_WF_{nm}', 'n': '', 'PF': '', 'cumR': '', 'mdd': '', 'fin': '',
                     'train_PF': round(best_tr, 3), 'test_PF': round(te_pf, 3), 'gap': '',
                     'flips': '', 'note': f"train최적ER{best_ge}(PF{best_tr:.2f}) → test PF{te_pf:.2f} vs 원본test PF{off_te:.2f}"})

    # ── 시나리오6: ER 게이트 거래의 OI 분포 (무덤차단이 ER장만 작동?) ──
    tr45_dz = [t for t in tr_off if t['entry_t'] in df_tf.index]  # 원본 거래 기준
    note6 = f"ER0.45 거래수 {stats_of(tr45)['n']} vs 원본 {off_all['n']} (차단 {off_all['n']-stats_of(tr45)['n']}건)"
    rows.append({'cell': 'S6_OI분포_ER0.45', 'n': stats_of(tr45)['n'], 'PF': '', 'cumR': '', 'mdd': '',
                 'fin': '', 'train_PF': '', 'test_PF': '', 'gap': '', 'flips': '', 'note': note6})

    # ── 시나리오7: 큰거래 제거 견고성 (ER0.45) ──
    R45 = sorted([t['R'] for t in tr45])
    trimmed = R45[:-3] if len(R45) > 3 else R45
    gp = sum(r for r in trimmed if r > 0); gl = -sum(r for r in trimmed if r < 0)
    pf_t = round(gp / gl, 3) if gl > 0 else 999.0
    rows.append({'cell': 'S7_큰이익3제거', 'n': len(trimmed), 'PF': pf_t, 'cumR': round(sum(trimmed)*100,2),
                 'mdd': '', 'fin': '', 'train_PF': '', 'test_PF': '', 'gap': '', 'flips': '',
                 'note': f"상위3제거 PF{pf_t} (원래 {stats_of(tr45)['PF']}) — 급락안하면 견고"})

    # ── 시나리오8: ML 재판정 (연도일관성 가중) ──
    ml_rows = []
    for ge in ER_GRID:
        tr = er_results[ge]
        trn = stats_of(tr, [2023, 2024, 2025])['PF']; te = stats_of(tr, [2026])['PF']
        consist = sum(int(stats_of(tr, [y])['cumR'] >= off_year[y]['cumR']) for y in YEARS)
        ml_rows.append({'cell': f'ER{ge}', 'train_PF': trn, 'test_PF': te,
                        'consist_years': consist, 'fin': stats_of(tr)['fin']})
    # 연도일관성 가중 점수
    ml_out = []
    for r in ml_rows:
        gap = abs(r['train_PF'] - r['test_PF'])
        score = 0.0
        if r['train_PF'] > 1.0 and r['test_PF'] > 1.0:
            score += 30
            score += r['consist_years'] * 12          # 연도일관성 최대 48점
            if gap < 0.5: score += 22
            elif gap < 1.0: score += 10
        verdict = "견고(실력)" if score >= 70 else "보통" if score >= 50 else "취약(우연의심)"
        ml_out.append({**r, 'gap': round(gap, 3), 'ml_score': round(score, 1), 'ml_verdict': verdict})
    ml_out.sort(key=lambda x: -x['ml_score'])

    top = ml_out[0]
    # 고원 판정: 상위 점수가 인접 ER에 몰려있나
    top_ers = sorted([float(r['cell'][2:]) for r in ml_out if r['ml_score'] >= 70])
    plateau = "고원(인접ER 다같이 좋음=실력)" if len(top_ers) >= 3 else (
        "좁은봉우리(과최적화의심)" if len(top_ers) >= 1 else "견고ER없음")

    # S8 ML재판정 행들을 summary에 명시(검사·가독성)
    for r in ml_out:
        rows.append({'cell': f"S8_ML_{r['cell']}", 'n': '', 'PF': '', 'cumR': '', 'mdd': '',
                     'fin': r['fin'], 'train_PF': r['train_PF'], 'test_PF': r['test_PF'],
                     'gap': r['gap'], 'flips': '',
                     'note': f"일관{r['consist_years']}/4년 score{r['ml_score']} {r['ml_verdict']}"})

    verdict = (f"VERDICT ER심화 | 원본 fin{off_all['fin']:.0f} | ER최고={top['cell']} score={top['ml_score']} {top['ml_verdict']} "
               f"(train{top['train_PF']} test{top['test_PF']} 일관{top['consist_years']}/4년 gap{top['gap']}) | "
               f"{plateau} | 견고ER={top_ers}")
    print("[verdict] " + verdict)

    out = [{'cell': verdict}] + rows
    pd.DataFrame(out).to_csv(os.path.join(HERE, "er_summary.csv"), index=False, encoding='utf-8-sig')
    pd.DataFrame(yearly).to_csv(os.path.join(HERE, "er_yearly.csv"), index=False, encoding='utf-8-sig')
    ...
    for t in tr45:
        ...
        td.append({'side': t['side'], 'entry_t': t['entry_t'].strftime('%Y-%m-%d %H:%M'),
                   ...
                   'oi_z': round(float(oi_arr[bi]), 3) if bi >= 0 and not np.isnan(oi_arr[bi]) else '',
                   'er': round(float(sig['er'][bi]), 3) if bi >= 0 else ''})
    pd.DataFrame(td).to_csv(os.path.join(HERE, "er_trades.csv"), index=False, encoding='utf-8-sig')

    # ml 결과도 yearly 파일 끝에 별도 저장 안 하고 summary note로. 대신 metric에.
    with open(os.path.join(HERE, ".er_metric"), 'w', encoding='utf-8') as f:
        f.write(f"has_oi=1\nbars={len(df_tf)}\noff_fin={off_all['fin']}\n")
        f.write(f"top_er={top['cell']}\ntop_score={top['ml_score']}\ntop_consist={top['consist_years']}\n")
        f.write(f"plateau_ers={top_ers}\nverdict={top['ml_verdict']}\n")
    print(f"[save] er_summary.csv + er_yearly.csv + er_trades.csv")
