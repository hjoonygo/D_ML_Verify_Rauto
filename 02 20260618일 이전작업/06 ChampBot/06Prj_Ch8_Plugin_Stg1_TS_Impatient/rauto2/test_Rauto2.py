...
import bot_trendstack_impatient_king as TBI
...
SLOT = "R2"; STRAT = "성급왕TS"; BUCKET_7H = 420
...
def main():
    ...
    bot = TBI.TrendStackImpatientKingBot(); bot.on_init({})
    ...
    try:
        ...
        fresh = TBI.TrendStackImpatientKingBot(); fresh.on_init({})
    ...
    log(f"[동치] live≡replay(가드없는 vanilla): {match} — 성급왕은 1분 인트라바 손절가드로 vanilla replay와 의도적으로 다름(정상)")
    if err: log("\n[예외]\n" + exc)
    ...
    ok = (err == 0)   # 성급왕: 동치(vs vanilla replay)는 인트라바 가드로 의도적 차이라 PASS 기준서 제외(정보용)
