# -*- coding: utf-8 -*-
# [파일명] check_07Prj_Ch1_Slippage_stg5_MaeLiqLimitFix.py
# 코드길이: 약 130줄 | 내부버전: 07Prj_Ch1_stg5_MaeLiqLimitFix_v2 | 로직 전체 출력(축약/생략 없음)
# ---------------------------------------------------------------------------------------------
# [이 코드가 하는 일] stg5 MAE 격리튕김(버그2건수정) 결과 오염검사 + 분석txt + INDEX 한 줄.
...
# [In]  하위폴더의 stg5_maefix_summary.csv / stg5_maefix_ledger.csv / stg5_maefix_coverage.csv
...
NAME = "07Prj_Ch1_Slippage_stg5_MaeLiqLimitFix"
...
REQ_CSV = ["stg5_maefix_summary.csv", "stg5_maefix_ledger.csv", "stg5_maefix_coverage.csv"]
...
def main():
    ...
    summ = read_csv_safe(os.path.join(HERE, "stg5_maefix_summary.csv"))
    cov = read_csv_safe(os.path.join(HERE, "stg5_maefix_coverage.csv"))
    led = read_csv_safe(os.path.join(HERE, "stg5_maefix_ledger.csv"))
    ...
    if led is not None and 'mae' in led.columns:
        ...
        if n_dup > 0:
            w("    [경고] 원장 중복 거래 발견!")
        # ★stg5 정합성: 손실거래의 MAE는 최종 R보다 깊거나 같아야(MAE<=R). 위반=청산봉 누락 버그 재발.
        loss = led[led['R'] < 0]
        viol = loss[loss['mae'] > loss['R'] + 1e-9]
        w(f"    ★MAE 정합성(손실거래 MAE<=R): 위반 {len(viol)}/{len(loss)}건 "
