# [check_07Prj_Ch4_RunAWS_Stg2_NBoostSweep.py]
# 코드길이: 약 120줄 / 내부버전: ch4_stg2_nboost_check_v1 / 로직 축약·생략 없이 전체 출력
# ─────────────────────────────────────────────────────────────────────────
# [목적] 오염검사 + 작업분석 저장 + INDEX 기록. (복붙 요청 없이 결과 전량 파일로)
#   (1) 오염검사 : 근간 데이터(devledger)·원본 로직(bot·poc)·고정파일 SHA256 무수정 + 중복/누락.
#   (2) 분석저장 : ..\00WorkHstr\(YYYYMMDDHHMM).txt — N_BOOST 스윕·CPCV 견고성 판정.
...
def main():
    ...
    detail = []
    for r in rows:
        detail.append(f"  N_BOOST={r['N_BOOST']:>3}  full {r['full_ret']}%/MDD{r['full_mdd']}%  "
                      f"CPCV최악MDD {r['cpcv_worst_mdd']}% 최악수익 {r['cpcv_worst_ret']}% 한도위반 {r['cpcv_breach']}")
    qualified = [r for r in rows if float(r['full_mdd']) > LIMIT
                 and float(r['cpcv_worst_mdd']) > LIMIT and float(r['cpcv_worst_ret']) > 0]
    if qualified:
        rec = max(qualified, key=lambda r: float(r['full_ret']))
        verdict_txt = f"권장 N_BOOST={rec['N_BOOST']} (CPCV 견고)"
    else:
        verdict_txt = "권장 N_BOOST=1.0 유지 — 상향은 CPCV 비견고(부분구간 MDD 한도초과)"

    stamp = datetime.now().strftime("%Y%m%d%H%M")
    txt_path = os.path.join(WH, f"{stamp}.txt")
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write(f"작업분석 — {NAME}\n초기자본 $10,000 복리 · devledger 264 실거래 재구성(절대수치 근사, N_BOOST 상대비교 목적)\n\n")
        f.write("\n".join(lines) + "\n\n[2] N_BOOST 스윕 · CPCV 견고성\n" + "\n".join(detail) + "\n\n")
        f.write(f"[판정] 오염검사 {'OK' if ok_all else 'FAIL'} · {verdict_txt}\n")

    idx_path = os.path.join(WH, "00WorkHstr_INDEX.txt")
    line = (f"{stamp} | {NAME} | CHECK N_BOOST1.0~1.4 CPCV | {verdict_txt} | "
