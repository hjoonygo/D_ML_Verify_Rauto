# -*- coding: utf-8 -*-
# [FILE] check_*.py (stg11 - champion-mix regime-switching integrity check)
...
BASE = "04All_IDEA4Concept_ch9_SideWayDCA_Alpha_stg11"
VER = BASE
REQUIRED = [f"test_{BASE}.py", f"check_{BASE}.py", f"run_{BASE}.bat",
...
def check_8():
    ...
    hashes = {f: sha256(os.path.join(HERE, f))[:16] for f in REQUIRED[:2] if present.get(f)}
    ...
    with open(os.path.join(HERE, f"test_{BASE}.py"), encoding="utf-8") as f:
        src = f.read()
    if "shift(-" in src:
    ...
    v = parse_verdict()
    res.append(("8.VERDICT존재", v is not None, (v[:60] + "...") if v else "없음"))
...
def write_analysis(passed, res, hashes, verdict):
    ...
    # 장세별 분해 (stg11 추가)
    tp = os.path.join(HERE, "sdca_trades.csv")
    if os.path.exists(tp):
        try:
            t = pd.read_csv(tp)
            if 'regime' in t.columns and len(t) > 0:
                lines.append("\n[장세별 거래 분포 (best 채택안)]")
                for rg in ['uptrend','downtrend','range']:
                    sub = t[t.regime==rg]
                    if len(sub):
                        lines.append(f"  {rg}: n{len(sub)} R합{sub.R_pct.sum():.2f}% 평균{sub.R_pct.mean():.4f}%")
        except Exception:
            pass
...
def main():
    ...
    update_index(f"{stamp} | {VER} | {'PASS' if passed else 'FAIL'} | {verdict[:140]}")
