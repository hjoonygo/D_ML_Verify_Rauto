...
**"휩소 넘기" 진단은 성공(랠리 역주행이 진짜 병목), 그러나 L2 지렛대는 COMBO 위에서 중복=폐기.** ★정정(Stg6 §19, 캡틴 지적): COMBO의 early_tp가 이미 랠리를 청산 쪽에서 수익화 → L2 진입-사이징은 중복이라 같은 안전사이징선 오히려 수익 −93%p 악화(RevoiRally 제거). **진짜 개선(수익+안전 동시)은 추세봇 상보가 유일(4차확인).** ★핵심교훈=봇 위 오버레이는 기존 봇이 이미 그 일을 하는지 먼저 확인.

## 0-1. ★정정 로그 (신뢰 기록)
- 초판(Stg1~5): L2를 검증 지렛대로 판단, RevoiRally@ETF 등록·배포(커밋 fa6adc1). **과대평가**.
- 정정(Stg6): 캡틴이 "두 봇 지표 같고 rally만 수익 낮다"고 지적 → §19 정밀비교로 L2가 COMBO 위에서 중복임을 확인 → RevoiRally 제거. 엔진 PlugIn만 inert 보존.
