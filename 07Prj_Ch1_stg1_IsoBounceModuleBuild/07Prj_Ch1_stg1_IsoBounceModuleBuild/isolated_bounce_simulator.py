...
#   liq_distance(기본 -0.0719): 청산까지 가격변동률 거리. (1/13)−MMR ≈ 7.69%−0.5% = 7.19%.
#       ※Binance Brackets 표 실측값으로 사장님 PC에서 확정 권장(현재 추론 75%).
...
MODE_PRESETS = {
    ...
    "M3_iso_tailcut":  dict(exposure=0.975, tail_cut=-0.075, liq_distance=-0.0719, enable_tail_cut=True),
}
...
ALPHA_PROVENANCE = {
    ...
    "numbers_need_verification": [
        "exposure=0.975 (7.5%*13 = sample value)",
        "liq_distance=-0.0719 (1/13 - MMR0.5% = Binance Tier1 assumption, verify with Brackets table)",
        "tail_cut=-0.075 (initial margin 7.5%; ignores liquidation fee ~0.5%)",
    ],
...
class IsoBounceSim:
    ...
    def __init__(self, exposure=0.975, tail_cut=-0.075, liq_distance=-0.0719, enable_tail_cut=True):
...
if __name__ == "__main__":
    ...
    # 경계 직전: R=-0.07 → 평시
    assert abs(m3.transform_R(-0.07) - (-0.06825)) < 1e-9, "M3 경계 직전 -7% → 평시 실패"
    # 경계 정확: R=-0.0719 → 테일컷
    assert abs(m3.transform_R(-0.0719) - (-0.075)) < 1e-9, "M3 경계 정확 -7.19% → 테일컷 실패"
    ...
    m3_off = IsoBounceSim(exposure=0.975, tail_cut=-0.075, liq_distance=-0.0719, enable_tail_cut=False)
