# -*- coding: utf-8 -*-
"""
[파일명] tbm_simulator_v8.py
코드길이: 약 570줄, 내부버전명: v8.0 (stage_1), 로직 축약/생략 없이 전체 출력.

[목적] v7 + 안 X 변경: fib_trigger을 ATR 기반 동적화 (Stage 1)

[v7 -> v8 변경 사항]
  * FIB_TRIGGER 상수(0.012 고정)를 FIB_TRIGGER_DEFAULT로 변경 (fallback용)
  * 새 인자: fib_trigger_atr_multi (진입 시점 ATR×N으로 fib_trigger 거리 계산)
  * 변경: fib_trigger_price 계산 - 동적 ATR 기반
  * 추가: result에 'fib_trigger_atr_multi', 'fib_trigger_dist' 컬럼

[다른 변경 없음] v7의 모든 로직 (Phase 1/2, fib_lock, 동적 hard SL, 2h reversal)은 그대로
...
[상수]
  ...
  SL_GATE = 0.0016
  TP_GATE = 0.0024
  FIB_TRIGGER_DEFAULT = 0.012 (★ 변경: fallback only, 기본은 ATR 기반)
  FIB_EXT = 0.618
  ATR_K = 0.5
  ATR_PERIOD = 20
  HARD_SL_ROE = 0.03
  ...
  DEFAULT_FIB_TRIGGER_ATR_MULTI = 3.0 (★ 신규: ATR×3 ≈ 현재 1.2% 수준)

[함수 In/Out]
  compute_atr(high, low, close, period) -> np.ndarray  (변경 없음)
  
  simulate_position_v8(entry_signal_idx_1m, side, df_1m, df_ob_tf, df_2h,
                        atr_ob_tf, leverage, w, N, timeout_bars_ob_tf,
                        ob_tf_minutes, enable_2h_reversal, regime_master,
                        atr_at_entry_pct, atr_multiplier,
                        use_dynamic_hard_sl,
                        fib_trigger_atr_multi)                # ★ 신규
    -> dict
    
  batch_simulate_v8(long_signal_indices_1m, short_signal_indices_1m,
                     df_1m, df_ob_tf, df_2h, atr_ob_tf,
                     atr_15m_pct_per_1m,
                     leverage, w, N, timeout_bars_ob_tf, ob_tf_minutes,
                     enable_2h_reversal, regime_master,
                     atr_multiplier, use_dynamic_hard_sl,
                     fib_trigger_atr_multi,                   # ★ 신규
...
SL_GATE = 0.0016
TP_GATE = 0.0024
FIB_TRIGGER_DEFAULT = 0.012  # fallback (ATR 정보 없을 때만)
FIB_EXT = 0.618
ATR_K = 0.5
ATR_PERIOD = 20
HARD_SL_ROE = 0.03
...
# ★ 신규 상수 (Stage 1, 안 X)
DEFAULT_FIB_TRIGGER_ATR_MULTI = 3.0  # ATR×3 ≈ 0.012 (= 기존 고정값 수준)
...
def simulate_position_v8(
    ...
    regime_master = None,
    atr_at_entry_pct: float = 0.004,
    atr_multiplier: float = DEFAULT_ATR_MULTIPLIER,
    use_dynamic_hard_sl: bool = USE_DYNAMIC_HARD_SL,
    fib_trigger_atr_multi: float = DEFAULT_FIB_TRIGGER_ATR_MULTI,  # ★ 신규 (Stage 1)
) -> Dict[str, Any]:
    """
    v8 — 한 신호에 대한 시뮬레이션 1회. v7 + 안 X (fib_trigger ATR 기반).

    IN: (v7과 동일 + 1개 신규)
      fib_trigger_atr_multi: fib_lock 발동 임계선의 ATR 배수
                             기본 3.0 ≈ 1.2% (= v7 고정값과 동일)
                             Stage 1 그리드: 0.5/1.0/1.5/2.0
    OUT:
      result dict (v7 + 'fib_trigger_atr_multi', 'fib_trigger_dist')
    """
    result = {
        ...
        # ★ 신규 컬럼 (안 X, Stage 1)
        'fib_trigger_atr_multi': fib_trigger_atr_multi,
        'fib_trigger_dist': None,
    }
    ...
    # ★★★ 안 X 신규: fib_trigger 동적 계산 (Stage 1, early return 전에 항상 계산) ★★★
    if np.isfinite(atr_at_entry_pct) and atr_at_entry_pct > 0:
        fib_trigger_dist = atr_at_entry_pct * fib_trigger_atr_multi
    else:
        fib_trigger_dist = FIB_TRIGGER_DEFAULT  # fallback
    result['fib_trigger_dist'] = fib_trigger_dist
    ...
    # fib_trigger_dist는 위에서 이미 계산됨 (result['fib_trigger_dist'])
    if side == 'long':
        bep_plus = entry_price * (1 + BEP_PLUS_PCT)
        fib_trigger_price = entry_price * (1 + fib_trigger_dist)
    else:
        bep_plus = entry_price * (1 - BEP_PLUS_PCT)
        fib_trigger_price = entry_price * (1 - fib_trigger_dist)
...
def batch_simulate_v8(
    ...
    atr_multiplier: float = DEFAULT_ATR_MULTIPLIER,
    use_dynamic_hard_sl: bool = USE_DYNAMIC_HARD_SL,
    fib_trigger_atr_multi: float = DEFAULT_FIB_TRIGGER_ATR_MULTI,  # ★ 신규
    verbose: bool = True,
) -> pd.DataFrame:
    """
    v8 배치 시뮬레이션. v7 + 안 X.

    IN: (v7과 동일 + fib_trigger_atr_multi)
      fib_trigger_atr_multi: fib_lock 발동 임계선의 ATR 배수
                             Stage 1 그리드: 0.5/1.0/1.5/2.0
    OUT: DataFrame
    """
