# -*- coding: utf-8 -*-
# [파일명] test.py  (Stage1b)
# 코드길이: 약 320줄, 내부버전명: Stage1c_fibo05_floor32, 로직 축약/생략 없이 전체 출력
#
...
#
# [★결과 전량 파일저장] S1c_trades_*.csv + S1c_summary.csv. 화면 복붙 불필요.
...
from Exec_Fibo_v2 import Exec_Fibo_v2   # ★Fibo_v2: 1차익절후 곧장피보 + 0.5 + 32bp하한선
...
def main():
    print("[Stage1c] 게이트+검증청산엔진(다중OB계단)+복리위험7%+강제청산")
    ...
    for rr_min, bep_on, tag in CONFIGS:
        ...
        pd.DataFrame(trades).to_csv(os.path.join(HERE, f"S1c_trades_{tag}.csv"), index=False, encoding='utf-8-sig')
    ...
    pd.DataFrame(summary).to_csv(os.path.join(HERE, "S1c_summary.csv"), index=False, encoding='utf-8-sig')
    print("[저장] S1c_trades_*.csv (4) + S1c_summary.csv — 전량 파일저장 완료")
