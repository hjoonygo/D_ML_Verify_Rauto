...
class Exec_Fibo_v2:
    ...
    def check_exit(self, current_price: float, bot_state: dict, params: dict) -> dict:
        ...
        # ------------------------------------------------------------
            ...
            if not bot_state.get('reduced_once', False):
                if target_idx < len(bullish_obs):
                    ...
                    if current_price <= target_ob['mean']:
                        ...
                        # 잔량 손절 하한선 = 진입가 -32bp(숏 유리쪽). 가격이 이 위로 오면 손절.
                        bot_state['fib_stop'] = entry * (1 - 0.0032)
                        return {"action": "REDUCE_SHORT", "reason": f"1차 OB 타겟 도달: 분할익절 후 피보전환(하한선 진입가-32bp={bot_state['fib_stop']:.2f})"}
                    # 1차 도달 전: 초기 하드손절만
                    hard_sl = entry * (1 + (fib_sl_pct / leverage))
                    if current_price >= hard_sl:
            ...
            # [Phase 2] 1차익절 후 — 피보나치 계단식 (fib_ext=0.5) + 32bp 하한선 손절
            else:
                # 32bp 하한선: 가격이 진입가-32bp 위로 되돌아오면 손절(잔량 방어)
                floor_32 = entry * (1 - 0.0032)
                ...
                if roe >= fib_trigger or max_roe >= fib_trigger:
                    ...
                    # 피보 스탑은 32bp 하한선보다 위로만 올라감(둘 중 유리한=낮은 쪽 채택)
                    new_stop = min(prev if prev is not None else float('inf'), fib_lock)
                    bot_state['fib_stop'] = new_stop
                    if current_price >= bot_state['fib_stop']:
                        return {"action": "CLOSE_SHORT", "reason": f"Fibonacci 계단식 스탑 터치 (락인 {fib_ext:.3f}: {bot_state['fib_stop']:.2f})"}
                else:
                    # 피보 미발동 구간: 32bp 하한선이 손절선
                    if current_price >= floor_32:
                        return {"action": "CLOSE_SHORT", "reason": f"피보 하한선(-32bp) 터치 ({floor_32:.2f})"}
