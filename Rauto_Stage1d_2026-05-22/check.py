...
#   3 파일명 불일치(오타/버전)      : 고정 패턴 S1d_trades_*.csv / S1d_summary.csv ?
...
TRADES_GLOB = "S1d_trades_*.csv"
SUMMARY = "S1d_summary.csv"
...
def run_checks(start_ts):
    ...
    # [3] 파일명 패턴
    bad = [os.path.basename(f) for f in trade_files if not os.path.basename(f).startswith("S1d_trades_")]
    ...
    with open(INDEX, 'a', encoding='utf-8') as f:
        ...
        if all_pass:
            f.write(f"{analysis['time']} | {ZIP_NAME} | 분석:{stamp}.txt | "
                    f"테스트:test.py | 결과:S1d_summary.csv | "
