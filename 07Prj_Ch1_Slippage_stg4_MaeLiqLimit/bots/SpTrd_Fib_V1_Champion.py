# -*- coding: utf-8 -*-
# [파일명] SpTrd_Fib_V1_Champion.py
# 코드길이: 약 700줄 | 내부버전: SpTrd_Fib_V1_Champion (06_Ch4 통합) | 로직 축약/생략 없이 전체 출력
# ─────────────────────────────────────────────────────────────────────────────
# [이 코드가 하는 일 — 통합 봇 (Ch4 최종 후보)]
#   Ch4에서 검증된 추세선수 알파업 3개를 하나로 합친 '최종 후보 봇'이다:
#     ① OI 무덤필터: 진입봉 oi_zscore_24h 가 [0,1)이면 진입 보류 (Stg2 p=0.0001 입증)
#     ② ER0.45 장세게이트: 무덤필터를 ER>=0.45 추세장에만 가동 (Stg5 견고4/4년 입증)
#     ③ 분할진입 A 3분할: 피보 되돌림마다 1/3씩 진입, 평단개선 (Stg6 PF1.76 입증)
#     + 피보 트레일링 현행 (0.3,0.5,0.6) — Stg6서 최적 재확인
#   ★중요: 각 조각은 검증됐으나 '셋을 합친 전체'는 통합 후 첫 백테스트다. Stg6(PF1.76,누적197%) 재현 확인 필수.
#   TF 7h 고정. 실효노출(수량×레버리지)은 챔피언 통합 후 결정 — 여기선 자본30%×1x 기준.
#
# [★사용명칭 정의]
#   무덤구간 = oi_zscore_24h [0,1). ER = |끝-시작|/Σ|봉변화| (20봉), 게이트 0.45.
#   분할A = 피보 되돌림(0.382/0.5/0.618) 도달마다 분할 체결, 평단가 진입.
#
# [미래참조 차단] ER/ADX/피벗/OI 모두 진입봉까지 과거값, 종가체결, 분할 i+k는 진입후 실제체결 모사. 음수shift 없음.
# [PATH] 실행: D:\ML\verify\(폴더)\ . 데이터: 상위 D:\ML\verify\ .
# [DATA] (상위) Merged_Data_with_Regime_Features.csv(OHLC+adx) / Merged_Data.csv(oi_zscore)
# [OUTPUT] (실행폴더) champ_summary.csv + champ_trades.csv + champ_yearly.csv
# [FUNCTIONS] compute_signals/run_strategy/compute_split_entry/stats_of/equity_s4/equity_risk (Stg6 계승)
# ─────────────────────────────────────────────────────────────────────────────
# ─────────────────────────────────────────────────────────────────────────────
...
def main():
    print("[SpTrd_Fib_V1_Champion] 통합봇: 무덤필터+ER0.45게이트+분할A3+현행피보 — 최종후보 단일실행")
    ...
    if not has_oi:
        print("[oi] ★Merged_Data.csv(oi_zscore) 없음 → 통합봇은 OI 필수. 중단."); 
        pd.DataFrame([{'cell': '★검증불가: oi_zscore 없음(상위 Merged_Data.csv 확인)'}]).to_csv(
            os.path.join(HERE, "champ_summary.csv"), index=False, encoding='utf-8-sig')
        return
    ...
    print(f"[config] ER게이트0.45 + 무덤[0,1) + 분할A3 + 피보(0.3,0.5,0.6)")

    # 통합봇 실행
    tr = run_strategy(df_tf, sig, 0, 'none', 0.8, **FINAL)
    # 비교용: 원본(무필터)
    tr_base = run_strategy(df_tf, sig, 0, 'none', 0.8, dz_oi=None)

    YEARS = sorted(set(t['year'] for t in tr_base))
    a = stats_of(tr); b = stats_of(tr_base)

    rows = []
    def addrow(cell, s, note=""):
        rows.append({'cell': cell, 'n': s['n'], 'PF': s['PF'], 'cumR': s['cumR'],
                     'win': s['win'], 'payoff': s['payoff'], 'mdd': s['mdd'],
                     'fin': s['fin'], 'flips': s['flips'], 'note': note})

    addrow("원본_무필터", b, "비교 기준선")
    addrow("통합봇_최종후보", a, "ER0.45+무덤+분할A3+현행피보")

    # 연도별
    yearly = []
    for y in YEARS:
        ya = stats_of(tr, [y]); yb = stats_of(tr_base, [y])
        yearly.append({'year': y, 'champ_cumR': ya['cumR'], 'base_cumR': yb['cumR'],
                       'champ_PF': ya['PF'], 'base_PF': yb['PF'], 'champ_mdd': ya['mdd'],
                       'better': int(ya['cumR'] >= yb['cumR'])})
    cons = sum(r['better'] for r in yearly)

    # train/test
    trn = stats_of(tr, [2023, 2024, 2025]); te = stats_of(tr, [2026])

    # 실효노출 표 (수량×레버리지 = 하나의 축)
    expo = []
    for cap_pct, lev, lab in [(0.30, 1.0, "30%(30%x1)"), (0.30, 2.0, "60%(30%x2)"),
                              (0.30, 3.0, "90%(30%x3)"), (0.30, 5.0, "150%(30%x5)")]:
        fin, mdd, bust = equity_risk(tr, cap_pct, lev)
        expo.append({'expo': lab, 'fin': fin, 'mdd': mdd, 'bust': bust})

    verdict = (f"VERDICT 통합봇 | 원본 PF{b['PF']} cumR{b['cumR']}% MDD{b['mdd']}% fin{b['fin']:.0f} "
               f"-> 통합 PF{a['PF']} cumR{a['cumR']}% MDD{a['mdd']}% fin{a['fin']:.0f} | "
               f"train PF{trn['PF']} test PF{te['PF']} | 연도일관 {cons}/{len(YEARS)}년 | "
               f"★Stg6목표(PF1.76 누적197%) 재현확인 | 실효노출표 참조")
    print("[verdict] " + verdict)

    out = [{'cell': verdict}] + rows
    out.append({'cell': '─ 실효노출(수량×레버리지) ─'})
    for e in expo:
        out.append({'cell': f"노출_{e['expo']}", 'fin': e['fin'], 'mdd': e['mdd'], 'note': f"파산{e['bust']}"})
    pd.DataFrame(out).to_csv(os.path.join(HERE, "champ_summary.csv"), index=False, encoding='utf-8-sig')
    pd.DataFrame(yearly).to_csv(os.path.join(HERE, "champ_yearly.csv"), index=False, encoding='utf-8-sig')

    td = [{'side': t['side'], 'entry_t': t['entry_t'].strftime('%Y-%m-%d %H:%M'),
           'exit_t': t['exit_t'].strftime('%Y-%m-%d %H:%M'), 'year': t['year'],
           'R_pct': round(t['R']*100, 4), 'reason': t['reason']} for t in tr]
    pd.DataFrame(td).to_csv(os.path.join(HERE, "champ_trades.csv"), index=False, encoding='utf-8-sig')
    print(f"[save] champ_summary.csv + champ_yearly.csv + champ_trades.csv")
