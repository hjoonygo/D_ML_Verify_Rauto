# -*- coding: utf-8 -*-
# [파일명] test_07Prj_Ch1_Slippage_stg4_MaeLiqLimit.py
# 코드길이: 약 250줄 | 내부버전: 07Prj_Ch1_stg4_MaeLiqLimit_v1 | 로직 전체 출력(축약/생략 없음)
# ---------------------------------------------------------------------------------------------
...
# [사용명칭 정의] MAE=거래 도중 진입가 대비 최저 불리변동(롱=최저가, 숏=최고가) / 청산거리 d=1/L-MMR-FEE
...
# [출력 CSV] (실행 하위폴더)
#   stg4_mae_summary.csv  : 5설정 × (격리튕김 ON/OFF 잔고·MDD·청산수·회복/직행 분해)
#   stg4_mae_ledger.csv   : 거래별 원장 + entry_price + mae (MAE 포함 원장, 다음 작업자용)
#   stg4_mae_coverage.csv : 실행 메타(거래수·1분봉수·펀딩·FNG커버)
# ---------------------------------------------------------------------------------------------
# [함수 In/Out]
...
#   compute_mae(kept,df1m)            In: 거래·1분봉                Out: 거래에 mae 추가
#   sim_liqlimit(kept,entry_pct,lev)  In: 거래·진입%·레버리지        Out: ON/OFF 잔고·MDD·청산·분해 dict
#   main()                            In: -                        Out: CSV 3종
# [변수] START=10000 MMR=0.005 FEE_LIQ=0.0125 HALF_COST=0.0007 COST_RT=0.0014
...
START = 10000.0
MMR = 0.005          # 유지증거금률
FEE_LIQ = 0.0125     # 강제청산 수수료 1.25% (청산거리 계산에 반영)
...
def compute_mae(kept, df1m):
    ...
    for t in kept:
        ...
        x = np.int64(pd.Timestamp(t['exit_t']).value)
        ...
        if t['side'] == 1:          # 롱: 최저가가 최악
            t['mae'] = float((low[i0:i1].min() - ep) / ep)
        else:                       # 숏: 최고가가 최악
            t['mae'] = float((ep - high[i0:i1].max()) / ep)
    return kept
...
def sim_liqlimit(kept, entry_pct, lev):
    ...
    EXP = entry * lev
    LIQ = -(1.0 / lev - MMR - FEE_LIQ)   # 청산거리(음수). 청산수수료 1.25% 반영.
    ...
    return dict(setting=f"{entry_pct}%x{lev}", entry_pct=entry_pct, lev=lev,
                EXP=round(EXP, 3), LIQ_dist=round(LIQ * 100, 2), single_loss=-entry_pct,
...
def main():
    print("[07Prj_Ch1 stg4] MAE 기반 격리튕김 5설정 검증 (방식A·14bp·청산수수료1.25%·실펀딩)")
    ...
    if DATA is None or FNG is None:
        pd.DataFrame([{'x': f'missing DATA={DATA} FNG={FNG}'}]).to_csv(
            os.path.join(HERE, "stg4_mae_summary.csv"), index=False, encoding='utf-8-sig')
    ...
    pd.DataFrame(rows).to_csv(os.path.join(HERE, "stg4_mae_summary.csv"), index=False, encoding='utf-8-sig')
    ...
    led.to_csv(os.path.join(HERE, "stg4_mae_ledger.csv"), index=False, encoding='utf-8-sig')
    ...
    pd.DataFrame([dict(n_trades=len(kept), n_1min_bars=len(df1m), n_7h_bars=len(idx7),
        ...
        os.path.join(HERE, "stg4_mae_coverage.csv"), index=False, encoding='utf-8-sig')

    print("[완료] stg4_mae_summary.csv / stg4_mae_ledger.csv / stg4_mae_coverage.csv 저장")
    for r in rows:
