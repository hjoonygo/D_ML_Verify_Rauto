# -*- coding: utf-8 -*-
# [파일명] test_06Prj_Ch4_SidewayDCA4RAUTO_Stg4.py
# 코드길이: 약 620줄 | 내부버전: ChampBot_SidewayDCA4RAUTO_06_Ch4_Stg4 | 로직 축약/생략 없이 전체 출력
# ─────────────────────────────────────────────────────────────────────────────
...
def main():
    print("[ChampBot_SidewayDCA4RAUTO_06_Ch4_Stg4] 장세판단 최적화 - 무덤필터 추세장정의 ML판정")
    ...
    if not has_oi:
        pd.DataFrame([{'cell': '★검증불가: oi_zscore 없음(상위 Merged_Data.csv 확인)'}]).to_csv(
            os.path.join(HERE, "sf4_summary.csv"), index=False, encoding='utf-8-sig')
        pd.DataFrame([{'x': ''}]).to_csv(os.path.join(HERE, "sf4_trades.csv"), index=False, encoding='utf-8-sig')
        pd.DataFrame([{'x': ''}]).to_csv(os.path.join(HERE, "sf4_mljudge.csv"), index=False, encoding='utf-8-sig')
        with open(os.path.join(HERE, ".sf4_metric"), 'w', encoding='utf-8') as f:
            f.write("has_oi=0\n")
        print("[save] ★OI없음 - 검증불가")
        return
    oi_arr = load_oi_8h(oipath, df_tf.index)
    bb_arr = load_bb_8h(data, df_tf.index)
    has_bb = bb_arr is not None and np.isfinite(bb_arr).any()
    print(f"[oi] 무덤봉 {int(np.sum((~np.isnan(oi_arr))&(oi_arr>=DZ_LO)&(oi_arr<DZ_HI)))} | bb {'O' if has_bb else 'X'}")

    TRAIN = [2023, 2024, 2025]; TEST = [2026]
    base = dict(adx_th=0, mode='none', atr_mult=0.8)

    def run_gate(gm, ga=ADX_TREND, ge=ER_TREND):
        return run_strategy(df_tf, sig, base['adx_th'], base['mode'], base['atr_mult'],
                            dz_oi=oi_arr, gate_mode=gm, gate_adx=ga, gate_er=ge, gate_bb=bb_arr)

    rows = []

    def add(cell, tr, note=""):
        all_s = stats_of(tr); trn = stats_of(tr, TRAIN); te = stats_of(tr, TEST)
        rows.append({'cell': cell, 'all_n': all_s['n'], 'all_PF': all_s['PF'], 'all_cumR': all_s['cumR'],
                     'all_mdd': all_s['mdd'], 'all_fin': all_s['fin'],
                     'train_PF': trn['PF'], 'train_cumR': trn['cumR'],
                     'test_PF': te['PF'], 'test_cumR': te['cumR'], 'test_mdd': te['mdd'],
                     'flips': all_s['flips'], 'note': note})

    # ── 시나리오1: 원본(필터OFF) 기준선 ──
    tr_off = run_strategy(df_tf, sig, base['adx_th'], base['mode'], base['atr_mult'], dz_oi=None)
    add("S1_원본_필터OFF", tr_off, "기준선")

    # ── 시나리오2: 무덤 × 항상ON (Stg3 방식, gate=none) ──
    add("S2_무덤_항상ON", run_gate('none'), "Stg3 방식")

    # ── 시나리오3: 무덤 × ADX>=25 ──
    add("S3_무덤×ADX25", run_gate('adx', ga=25.0))

    # ── 시나리오4: 무덤 × ER>=0.4 ──
    add("S4_무덤×ER0.4", run_gate('er', ge=0.40))

    # ── 시나리오5: 무덤 × (ADX25 AND BB확장) ──
    if has_bb:
        add("S5_무덤×ADX_BB", run_gate('adx_bb', ga=25.0))
    else:
        rows.append({'cell': 'S5_무덤×ADX_BB ★불가(bb없음)', 'all_n': '', 'all_PF': '', 'all_cumR': '',
                     'all_mdd': '', 'all_fin': '', 'train_PF': '', 'train_cumR': '',
                     'test_PF': '', 'test_cumR': '', 'test_mdd': '', 'flips': '', 'note': 'bb_width_pct 없음'})

    # ── 시나리오6: 추세문턱 스윕 ──
    for ga in [22.0, 25.0, 28.0]:
        add(f"S6_ADX스윕_{ga}", run_gate('adx', ga=ga))
    for ge in [0.35, 0.40, 0.45]:
        add(f"S6_ER스윕_{ge}", run_gate('er', ge=ge))

    # ── 시나리오7: ML 판정 (주요 정의들의 견고성 점수) ──
    judge_rows = [r for r in rows if str(r['cell']).startswith(('S2_', 'S3_', 'S4_', 'S5_', 'S6_'))
                  and isinstance(r.get('train_PF'), (int, float))]
    ml_out = ml_judge(judge_rows)

    # ── 시나리오8: 연도별 (best 정의에서 2025·2026 악화 사라지나) ──
    best = ml_out[0] if ml_out else None
    yearly_note = ""
    if best:
        # best 정의 재실행해서 연도별
        bc = best['cell']
        if 'ADX' in bc and '스윕' not in bc:
            tr_best = run_gate('adx', ga=25.0)
        elif 'ER' in bc and '스윕' not in bc:
            tr_best = run_gate('er', ge=0.40)
        elif 'ADX_BB' in bc:
            tr_best = run_gate('adx_bb', ga=25.0)
        elif '항상' in bc:
            tr_best = run_gate('none')
        else:
            tr_best = run_gate('adx', ga=25.0)
        for y in [2024, 2025, 2026]:
            ys = stats_of(tr_best, [y])
            yearly_note += f"{y}:PF{ys['PF']}cumR{ys['cumR']}% "

    # ── VERDICT ──
    top = ml_out[0] if ml_out else {}
    # S7·S8을 summary 행으로 명시(검사·가독성)
    rows.append({'cell': 'S7_ML판정', 'all_n': len(ml_out), 'all_PF': '', 'all_cumR': '',
                 'all_mdd': '', 'all_fin': '', 'train_PF': top.get('train_PF', ''),
                 'test_PF': top.get('test_PF', ''), 'test_cumR': '', 'test_mdd': '',
                 'train_cumR': '', 'flips': '',
                 'note': f"최고={top.get('cell','?')} score={top.get('ml_score','?')} {top.get('ml_verdict','?')} gap={top.get('gap','?')}"})
    rows.append({'cell': 'S8_연도별_best', 'all_n': '', 'all_PF': '', 'all_cumR': '',
                 'all_mdd': '', 'all_fin': '', 'train_PF': '', 'test_PF': '', 'test_cumR': '',
                 'test_mdd': '', 'train_cumR': '', 'flips': '', 'note': yearly_note})

    verdict = (f"VERDICT 장세판단 | 원본 PF{rows[0]['all_PF']} test{rows[0]['test_PF']} | "
               f"ML최고={top.get('cell','?')} score={top.get('ml_score','?')} {top.get('ml_verdict','?')} "
               f"(train PF{top.get('train_PF','?')} test PF{top.get('test_PF','?')} gap{top.get('gap','?')}) | "
               f"best연도별 {yearly_note}| OI O BB {'O' if has_bb else 'X'}")
    print("[verdict] " + verdict)

    out = [{'cell': verdict}] + rows
    pd.DataFrame(out).to_csv(os.path.join(HERE, "sf4_summary.csv"), index=False, encoding='utf-8-sig')
    pd.DataFrame(ml_out).to_csv(os.path.join(HERE, "sf4_mljudge.csv"), index=False, encoding='utf-8-sig')

    # best 정의 거래원장
    td = []
    for t in (tr_best if best else tr_off):
        bi = df_tf.index.get_loc(t['entry_t']) if t['entry_t'] in df_tf.index else -1
        z = oi_arr[bi] if (bi >= 0 and bi < len(oi_arr)) else np.nan
        td.append({'side': t['side'], 'entry_t': t['entry_t'].strftime('%Y-%m-%d %H:%M'),
                   ...
                   'adx': round(float(sig['adx'][bi]), 2) if bi >= 0 else '',
                   'er': round(float(sig['er'][bi]), 3) if bi >= 0 else ''})
    pd.DataFrame(td).to_csv(os.path.join(HERE, "sf4_trades.csv"), index=False, encoding='utf-8-sig')

    with open(os.path.join(HERE, ".sf4_metric"), 'w', encoding='utf-8') as f:
        f.write(f"has_oi=1\nhas_bb={int(has_bb)}\nbars={len(df_tf)}\n")
        f.write(f"off_PF={rows[0]['all_PF']}\noff_test_PF={rows[0]['test_PF']}\n")
        f.write(f"ml_best={top.get('cell','?')}\nml_score={top.get('ml_score','?')}\n")
        f.write(f"best_train_PF={top.get('train_PF','?')}\nbest_test_PF={top.get('test_PF','?')}\n")
        f.write(f"best_gap={top.get('gap','?')}\n")

    print(f"[save] sf4_summary.csv + sf4_trades.csv + sf4_mljudge.csv")
